...
local THEME_NAME="Luna-Impulse"
local LAYOUT_MODE="standard"
local TAB_BOTTOM=false
local TABS_ENABLED=true
local HEADER_FILE="Themes\\Luna-Impulse\\textures\\header.gif"
local HEADER_PATH="Themes/Luna-Impulse/textures/header.gif"
...
local TOGGLE_ON_FILE="Themes\\Luna-Impulse\\textures\\toggle_on.png"
local TOGGLE_ON_PATH="Themes/Luna-Impulse/textures/toggle_on.png"
local TOGGLE_ON_URL_PATH=""
local toggle_on_handle=0
local toggle_on_downloaded=false
local TOGGLE_OFF_FILE="Themes\\Luna-Impulse\\textures\\toggle_off.png"
local TOGGLE_OFF_PATH="Themes/Luna-Impulse/textures/toggle_off.png"
local TOGGLE_OFF_URL_PATH=""
local toggle_off_handle=0
local toggle_off_downloaded=false
local LIST_ICON_FILE="Themes\\Luna-Impulse\\textures\\list_icon.png"
local LIST_ICON_PATH="Themes/Luna-Impulse/textures/list_icon.png"
local LIST_ICON_URL_PATH=""
local list_icon_handle=0
local list_icon_downloaded=false
local UI_ASSETS={
    ["list"]={file="Themes\\Luna-Impulse\\textures\\ui_theme_list.png",path="Themes/Luna-Impulse/textures/ui_theme_list.png",url=""},
    ["search"]={file="Themes\\Luna-Impulse\\textures\\ui_theme_search.png",path="Themes/Luna-Impulse/textures/ui_theme_search.png",url=""},
    ["toggle off"]={file="Themes\\Luna-Impulse\\textures\\ui_theme_toggle_off.png",path="Themes/Luna-Impulse/textures/ui_theme_toggle_off.png",url=""},
    ["toggle on"]={file="Themes\\Luna-Impulse\\textures\\ui_theme_toggle_on.png",path="Themes/Luna-Impulse/textures/ui_theme_toggle_on.png",url=""},
}
...
local SETTINGS_FILE="stand_import_v3_luna-impulse.ini"
menu.clear_settings()
menu.add_setting_color("Stand Accent",178,28,151,255,"Imported primary colour")
menu.add_setting_color("Stand Background",8,10,7,255,"Imported background colour")
menu.add_setting_slider("Stand Width",462,120,1200,1,"Width in Stand's 1920x1080 HUD pixels")
...
menu.add_setting_slider("Stand Position X",1118,0,1920,1,"Main-list X coordinate in Stand HUD pixels")
menu.add_setting_slider("Stand Position Y",261,0,1080,1,"Main-list Y coordinate in Stand HUD pixels")
...
local function load_settings()
    ...
    for line in body:gmatch("[^\r\n]+") do
        ...
        elseif key=="width" then menu.set_setting("Stand Width",tonumber(value) or 462)
        ...
        elseif key=="position_x" then menu.set_setting("Stand Position X",tonumber(value) or 1118)
        elseif key=="position_y" then menu.set_setting("Stand Position Y",tonumber(value) or 261)
        end
    end
end

local function serialize_settings()
    local accent=setting("Stand Accent") or {r=178,g=28,b=151,a=255}
    local background=setting("Stand Background") or {r=8,g=10,b=7,a=255}
    local width=setting("Stand Width") or {f_val=462}
    local rows=setting("Stand Rows") or {f_val=12}
    local row_height=setting("Stand Row Height") or {f_val=31}
    local position_x=setting("Stand Position X") or {f_val=1118}
    local position_y=setting("Stand Position Y") or {f_val=261}
    return table.concat({
...
local function reset_settings()
    menu.set_setting("Stand Accent",178,28,151,255)
    menu.set_setting("Stand Background",8,10,7,255)
    menu.set_setting("Stand Width",462)
    menu.set_setting("Stand Rows",12)
    menu.set_setting("Stand Row Height",31)
    menu.set_setting("Stand Position X",1118)
    menu.set_setting("Stand Position Y",261)
...
function draw_menu()
    ...
    local source_width=clamp(width_setting and width_setting.f_val or 462,120,1200)
    ...
    local position_x=clamp(position_x_setting and position_x_setting.f_val or 1118,0,1920)*hud_sx
    local position_y=clamp(position_y_setting and position_y_setting.f_val or 261,0,1080)*hud_sy
    ...
    if 0>0 and 0>0 then
        draw.rect_outline(x,content_y,x+width,content_y+body_h,0,0,0,0,5,0)
    end
    ...
    if page_h>0 then
        ...
        if TABS_ENABLED then
            ...
            text.draw(font.small,x+width-10-text.width(font.small,counter),
                page_y+(page_h-text.height(font.small))*0.5,28,255,255,255,counter)
        end
    end
        ...
        if item then
            ...
            if item.is_header then
                text.draw(font.small,x+10,yy+(row_h-text.height(font.small))*0.5,
                    active and 28 or accent.r,active and 255 or accent.g,active and 255 or accent.b,255,
                ...
                text.draw_ellipsis(font.item,text_x,yy+(row_h-text.height(font.item))*0.5,
                    active and 28 or 28,active and 255 or 255,active and 255 or 255,active and 255 or 255,
                ...
                if toggle_type then
                    ...
                    if toggle_handle and toggle_handle>0 then
                        ...
                        draw.rect_outline(indicator_x-5,yy+row_h*0.5-5,indicator_x+5,yy+row_h*0.5+5,
                            active and 28 or 28,active and 255 or 255,active and 255 or 255,210,1,1)
                        if item.on then draw.rect(indicator_x-3,yy+row_h*0.5-3,indicator_x+3,yy+row_h*0.5+3,
                            active and 28 or accent.r,active and 255 or accent.g,active and 255 or accent.b,255,1) end
                    end
                ...
                elseif action_mark then
                    ...
                    if action_handle and action_handle>0 then
                        ...
                        text.draw(font.small,x+width-10-text.width(font.small,mark),yy+(row_h-text.height(font.small))*0.5,
                            active and 28 or 28,active and 255 or 255,active and 255 or 255,255,mark)
                    end
                end
                if value~="" then
                    ...
                    text.draw(font.value,right-text.width(font.value,value),yy+(row_h-text.height(font.value))*0.5,
                        active and 28 or 28,active and 255 or 255,active and 255 or 255,255,value)
                end
            end
    ...
    if footer_handle and footer_handle>0 then
        ...
        text.draw(font.desc,x+9,desc_y+7,28,255,255,255,description,width-18)
        draw.pop_clip()
    end
