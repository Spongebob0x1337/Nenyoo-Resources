...
local THEME_NAME="X-Force"
local LAYOUT_MODE="standard"
local TAB_BOTTOM=false
local TABS_ENABLED=false
local HEADER_FILE="Themes\\X-Force\\textures\\header.bmp"
local HEADER_PATH="Themes/X-Force/textures/header.bmp"
...
local SUBHEADER_FILE="Themes\\X-Force\\textures\\subheader.bmp"
local SUBHEADER_PATH="Themes/X-Force/textures/subheader.bmp"
local SUBHEADER_URL_PATH=""
local subheader_handle=0
local subheader_downloaded=false
local FOOTER_FILE="Themes\\X-Force\\textures\\footer.bmp"
local FOOTER_PATH="Themes/X-Force/textures/footer.bmp"
...
local TOGGLE_ON_FILE="Themes\\X-Force\\textures\\toggle_on.png"
local TOGGLE_ON_PATH="Themes/X-Force/textures/toggle_on.png"
local TOGGLE_ON_URL_PATH=""
local toggle_on_handle=0
local toggle_on_downloaded=false
local TOGGLE_OFF_FILE="Themes\\X-Force\\textures\\toggle_off.png"
local TOGGLE_OFF_PATH="Themes/X-Force/textures/toggle_off.png"
local TOGGLE_OFF_URL_PATH=""
local toggle_off_handle=0
local toggle_off_downloaded=false
local LIST_ICON_FILE="Themes\\X-Force\\textures\\list_icon.png"
local LIST_ICON_PATH="Themes/X-Force/textures/list_icon.png"
local LIST_ICON_URL_PATH=""
local list_icon_handle=0
local list_icon_downloaded=false
local UI_ASSETS={
    ["list"]={file="Themes\\X-Force\\textures\\ui_theme_list.png",path="Themes/X-Force/textures/ui_theme_list.png",url=""},
    ["toggle off"]={file="Themes\\X-Force\\textures\\ui_theme_toggle_off.png",path="Themes/X-Force/textures/ui_theme_toggle_off.png",url=""},
    ["toggle on"]={file="Themes\\X-Force\\textures\\ui_theme_toggle_on.png",path="Themes/X-Force/textures/ui_theme_toggle_on.png",url=""},
}
...
local SETTINGS_FILE="stand_import_v3_x-force.ini"
menu.clear_settings()
menu.add_setting_color("Stand Accent",52,135,106,255,"Imported primary colour")
menu.add_setting_color("Stand Background",0,0,0,150,"Imported background colour")
menu.add_setting_slider("Stand Width",504,120,1200,1,"Width in Stand's 1920x1080 HUD pixels")
menu.add_setting_slider("Stand Rows",14,5,24,1,"Visible option rows")
...
menu.add_setting_slider("Stand Position Y",350,0,1080,1,"Main-list Y coordinate in Stand HUD pixels")
...
local function load_settings()
    ...
    for line in body:gmatch("[^\r\n]+") do
        ...
        elseif key=="width" then menu.set_setting("Stand Width",tonumber(value) or 504)
        elseif key=="rows" then menu.set_setting("Stand Rows",tonumber(value) or 14)
        ...
        elseif key=="position_y" then menu.set_setting("Stand Position Y",tonumber(value) or 350)
        end
    end
end

local function serialize_settings()
    local accent=setting("Stand Accent") or {r=52,g=135,b=106,a=255}
    local background=setting("Stand Background") or {r=0,g=0,b=0,a=150}
    local width=setting("Stand Width") or {f_val=504}
    local rows=setting("Stand Rows") or {f_val=14}
    local row_height=setting("Stand Row Height") or {f_val=31}
    local position_x=setting("Stand Position X") or {f_val=1240}
    local position_y=setting("Stand Position Y") or {f_val=350}
    return table.concat({
...
local function reset_settings()
    menu.set_setting("Stand Accent",52,135,106,255)
    menu.set_setting("Stand Background",0,0,0,150)
    menu.set_setting("Stand Width",504)
    menu.set_setting("Stand Rows",14)
    menu.set_setting("Stand Row Height",31)
    menu.set_setting("Stand Position X",1240)
    menu.set_setting("Stand Position Y",350)
...
function draw_menu()
    ...
    local source_width=clamp(width_setting and width_setting.f_val or 504,120,1200)
    local width=clamp(source_width*hud_sx,1,math.max(1,ctx.screen_w()-16))
    local requested_rows=math.floor(row_setting and row_setting.f_val or 14)
    ...
    local position_y=clamp(position_y_setting and position_y_setting.f_val or 350,0,1080)*hud_sy
