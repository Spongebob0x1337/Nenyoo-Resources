...
local THEME_NAME="Gradient Pulsive"
local LAYOUT_MODE="standard"
local TAB_BOTTOM=false
local TABS_ENABLED=true
local HEADER_FILE="Themes\\Gradient Pulsive\\textures\\header.png"
local HEADER_PATH="Themes/Gradient Pulsive/textures/header.png"
...
local SETTINGS_FILE="stand_import_v3_gradient_pulsive.ini"
menu.clear_settings()
menu.add_setting_color("Stand Accent",0,0,0,0,"Imported primary colour")
menu.add_setting_color("Stand Background",40,40,40,255,"Imported background colour")
menu.add_setting_slider("Stand Width",392,120,1200,1,"Width in Stand's 1920x1080 HUD pixels")
...
local function load_settings()
    ...
    for line in body:gmatch("[^\r\n]+") do
        ...
        elseif key=="width" then menu.set_setting("Stand Width",tonumber(value) or 392)
...
local function serialize_settings()
    ...
    local background=setting("Stand Background") or {r=40,g=40,b=40,a=255}
    local width=setting("Stand Width") or {f_val=392}
...
local function reset_settings()
    ...
    menu.set_setting("Stand Background",40,40,40,255)
    menu.set_setting("Stand Width",392)
...
function draw_menu()
    ...
    local source_width=clamp(width_setting and width_setting.f_val or 392,120,1200)
    ...
    if 255>0 and 4>0 then
        draw.rect_outline(x,content_y,x+width,content_y+body_h,40,40,40,255,5,4)
    end
    ...
    for row=0,visible-1 do
        ...
        if item then
            ...
            if 255>0 then
                draw.rect(x,yy+row_h-1,x+width,yy+row_h,40,40,40,math.min(255,80))
            end
            ...
            if item.is_header then
                ...
                text.draw_ellipsis(font.item,text_x,yy+(row_h-text.height(font.item))*0.5,
                    active and 255 or 152,active and 255 or 152,active and 255 or 155,active and 255 or 255,
                ...
                if toggle_type then
                    ...
                    if toggle_handle and toggle_handle>0 then
                        ...
                        draw.rect_outline(indicator_x-5,yy+row_h*0.5-5,indicator_x+5,yy+row_h*0.5+5,
                            active and 255 or 152,active and 255 or 152,active and 255 or 155,210,1,1)
                ...
                elseif action_mark then
                    ...
                    if action_handle and action_handle>0 then
                        ...
                        text.draw(font.small,x+width-10-text.width(font.small,mark),yy+(row_h-text.height(font.small))*0.5,
                            active and 255 or 152,active and 255 or 152,active and 255 or 155,255,mark)
                    end
                end
                if value~="" then
                    ...
                    text.draw(font.value,right-text.width(font.value,value),yy+(row_h-text.height(font.value))*0.5,
                        active and 255 or 152,active and 255 or 153,active and 255 or 155,255,value)
                end
            end
