...
local THEME_NAME="Capybara"
local LAYOUT_MODE="standard"
local TAB_BOTTOM=false
local TABS_ENABLED=true
local HEADER_FILE="Themes\\Capybara\\textures\\header.png"
local HEADER_PATH="Themes/Capybara/textures/header.png"
...
local SETTINGS_FILE="stand_import_v3_capybara.ini"
menu.clear_settings()
menu.add_setting_color("Stand Accent",241,210,163,255,"Imported primary colour")
menu.add_setting_color("Stand Background",255,255,255,255,"Imported background colour")
...
menu.add_setting_slider("Stand Row Height",29,12,100,1,"Row height in Stand HUD pixels")
menu.add_setting_slider("Stand Position X",255,0,1920,1,"Main-list X coordinate in Stand HUD pixels")
menu.add_setting_slider("Stand Position Y",264,0,1080,1,"Main-list Y coordinate in Stand HUD pixels")
...
local function load_settings()
    ...
    for line in body:gmatch("[^\r\n]+") do
        ...
        elseif key=="row_height" then menu.set_setting("Stand Row Height",tonumber(value) or 29)
        elseif key=="position_x" then menu.set_setting("Stand Position X",tonumber(value) or 255)
        elseif key=="position_y" then menu.set_setting("Stand Position Y",tonumber(value) or 264)
        end
    end
end

local function serialize_settings()
    local accent=setting("Stand Accent") or {r=241,g=210,b=163,a=255}
    local background=setting("Stand Background") or {r=255,g=255,b=255,a=255}
    local width=setting("Stand Width") or {f_val=420}
    local rows=setting("Stand Rows") or {f_val=16}
    local row_height=setting("Stand Row Height") or {f_val=29}
    local position_x=setting("Stand Position X") or {f_val=255}
    local position_y=setting("Stand Position Y") or {f_val=264}
    return table.concat({
...
local function reset_settings()
    menu.set_setting("Stand Accent",241,210,163,255)
    menu.set_setting("Stand Background",255,255,255,255)
    menu.set_setting("Stand Width",420)
    menu.set_setting("Stand Rows",16)
    menu.set_setting("Stand Row Height",29)
    menu.set_setting("Stand Position X",255)
    menu.set_setting("Stand Position Y",264)
...
function draw_menu()
    ...
    local row_h=clamp(row_height_setting and row_height_setting.f_val or 29,12,100)*hud_sy
    local position_x=clamp(position_x_setting and position_x_setting.f_val or 255,0,1920)*hud_sx
    local position_y=clamp(position_y_setting and position_y_setting.f_val or 264,0,1080)*hud_sy
