...
local THEME_NAME="Circuit"
local LAYOUT_MODE="standard"
local TAB_BOTTOM=false
local TABS_ENABLED=false
local HEADER_FILE="Themes\\Circuit\\textures\\header.bmp"
local HEADER_PATH="Themes/Circuit/textures/header.bmp"
...
local SUBHEADER_FILE="Themes\\Circuit\\textures\\subheader.bmp"
local SUBHEADER_PATH="Themes/Circuit/textures/subheader.bmp"
local SUBHEADER_URL_PATH=""
local subheader_handle=0
local subheader_downloaded=false
local FOOTER_FILE="Themes\\Circuit\\textures\\footer.bmp"
local FOOTER_PATH="Themes/Circuit/textures/footer.bmp"
...
local TOGGLE_ON_FILE="Themes\\Circuit\\textures\\toggle_on.png"
local TOGGLE_ON_PATH="Themes/Circuit/textures/toggle_on.png"
local TOGGLE_ON_URL_PATH=""
local toggle_on_handle=0
local toggle_on_downloaded=false
local TOGGLE_OFF_FILE="Themes\\Circuit\\textures\\toggle_off.png"
local TOGGLE_OFF_PATH="Themes/Circuit/textures/toggle_off.png"
local TOGGLE_OFF_URL_PATH=""
local toggle_off_handle=0
local toggle_off_downloaded=false
local LIST_ICON_FILE="Themes\\Circuit\\textures\\list_icon.png"
local LIST_ICON_PATH="Themes/Circuit/textures/list_icon.png"
local LIST_ICON_URL_PATH=""
local list_icon_handle=0
local list_icon_downloaded=false
local UI_ASSETS={
    ["list"]={file="Themes\\Circuit\\textures\\ui_theme_list.png",path="Themes/Circuit/textures/ui_theme_list.png",url=""},
    ["toggle off"]={file="Themes\\Circuit\\textures\\ui_theme_toggle_off.png",path="Themes/Circuit/textures/ui_theme_toggle_off.png",url=""},
    ["toggle off list"]={file="Themes\\Circuit\\textures\\ui_theme_toggle_off_list.png",path="Themes/Circuit/textures/ui_theme_toggle_off_list.png",url=""},
    ["toggle on"]={file="Themes\\Circuit\\textures\\ui_theme_toggle_on.png",path="Themes/Circuit/textures/ui_theme_toggle_on.png",url=""},
    ["toggle on list"]={file="Themes\\Circuit\\textures\\ui_theme_toggle_on_list.png",path="Themes/Circuit/textures/ui_theme_toggle_on_list.png",url=""},
}
...
local SETTINGS_FILE="stand_import_v3_circuit.ini"
menu.clear_settings()
menu.add_setting_color("Stand Accent",94,94,94,255,"Imported primary colour")
menu.add_setting_color("Stand Background",0,0,0,200,"Imported background colour")
menu.add_setting_slider("Stand Width",408,120,1200,1,"Width in Stand's 1920x1080 HUD pixels")
...
menu.add_setting_slider("Stand Position X",32,0,1920,1,"Main-list X coordinate in Stand HUD pixels")
menu.add_setting_slider("Stand Position Y",165,0,1080,1,"Main-list Y coordinate in Stand HUD pixels")
...
local function load_settings()
    ...
    for line in body:gmatch("[^\r\n]+") do
        ...
        elseif key=="width" then menu.set_setting("Stand Width",tonumber(value) or 408)
        ...
        elseif key=="position_x" then menu.set_setting("Stand Position X",tonumber(value) or 32)
        elseif key=="position_y" then menu.set_setting("Stand Position Y",tonumber(value) or 165)
        end
    end
end

local function serialize_settings()
    local accent=setting("Stand Accent") or {r=94,g=94,b=94,a=255}
    local background=setting("Stand Background") or {r=0,g=0,b=0,a=200}
    local width=setting("Stand Width") or {f_val=408}
    local rows=setting("Stand Rows") or {f_val=13}
    local row_height=setting("Stand Row Height") or {f_val=31}
    local position_x=setting("Stand Position X") or {f_val=32}
    local position_y=setting("Stand Position Y") or {f_val=165}
    return table.concat({
...
local function reset_settings()
    menu.set_setting("Stand Accent",94,94,94,255)
    menu.set_setting("Stand Background",0,0,0,200)
    menu.set_setting("Stand Width",408)
    menu.set_setting("Stand Rows",13)
    menu.set_setting("Stand Row Height",31)
    menu.set_setting("Stand Position X",32)
    menu.set_setting("Stand Position Y",165)
...
function draw_menu()
    ...
    local source_width=clamp(width_setting and width_setting.f_val or 408,120,1200)
    ...
    local position_x=clamp(position_x_setting and position_x_setting.f_val or 32,0,1920)*hud_sx
    local position_y=clamp(position_y_setting and position_y_setting.f_val or 165,0,1080)*hud_sy
    ...
    if page_h>0 then
        ...
        if TABS_ENABLED then
            ...
            text.draw(font.small,x+width-10-text.width(font.small,counter),
                page_y+(page_h-text.height(font.small))*0.5,168,168,168,255,counter)
        end
    end
        ...
        if item then
            ...
            if item.is_header then
                ...
                text.draw_ellipsis(font.item,text_x,yy+(row_h-text.height(font.item))*0.5,
                    active and 0 or 168,active and 0 or 168,active and 0 or 168,active and 255 or 255,
                ...
                if toggle_type then
                    ...
                    if toggle_handle and toggle_handle>0 then
                        ...
                        draw.rect_outline(indicator_x-5,yy+row_h*0.5-5,indicator_x+5,yy+row_h*0.5+5,
                            active and 0 or 168,active and 0 or 168,active and 0 or 168,210,1,1)
                ...
                elseif action_mark then
                    ...
                    if action_handle and action_handle>0 then
                        ...
                        text.draw(font.small,x+width-10-text.width(font.small,mark),yy+(row_h-text.height(font.small))*0.5,
                            active and 0 or 168,active and 0 or 168,active and 0 or 168,255,mark)
                    end
                end
                if value~="" then
                    ...
                    text.draw(font.value,right-text.width(font.value,value),yy+(row_h-text.height(font.value))*0.5,
                        active and 0 or 168,active and 0 or 168,active and 0 or 168,255,value)
                end
            end
    ...
    if footer_handle and footer_handle>0 then
        ...
        text.draw(font.desc,x+9,desc_y+7,168,168,168,255,description,width-18)
        draw.pop_clip()
    end
