...
local THEME_NAME="Off-White"
local LAYOUT_MODE="standard"
local TAB_BOTTOM=false
local TABS_ENABLED=true
local HEADER_FILE="Themes\\Off-White\\textures\\header.bmp"
local HEADER_PATH="Themes/Off-White/textures/header.bmp"
...
local SETTINGS_FILE="stand_import_v3_off-white.ini"
menu.clear_settings()
...
menu.add_setting_slider("Stand Width",476,120,1200,1,"Width in Stand's 1920x1080 HUD pixels")
...
menu.add_setting_slider("Stand Position X",191,0,1920,1,"Main-list X coordinate in Stand HUD pixels")
menu.add_setting_slider("Stand Position Y",43,0,1080,1,"Main-list Y coordinate in Stand HUD pixels")
...
local function load_settings()
    ...
    for line in body:gmatch("[^\r\n]+") do
        ...
        elseif key=="width" then menu.set_setting("Stand Width",tonumber(value) or 476)
        ...
        elseif key=="position_x" then menu.set_setting("Stand Position X",tonumber(value) or 191)
        elseif key=="position_y" then menu.set_setting("Stand Position Y",tonumber(value) or 43)
        end
    end
end
...
local function serialize_settings()
    ...
    local width=setting("Stand Width") or {f_val=476}
    local rows=setting("Stand Rows") or {f_val=14}
    local row_height=setting("Stand Row Height") or {f_val=30}
    local position_x=setting("Stand Position X") or {f_val=191}
    local position_y=setting("Stand Position Y") or {f_val=43}
    return table.concat({
...
local function reset_settings()
    ...
    menu.set_setting("Stand Width",476)
    menu.set_setting("Stand Rows",14)
    menu.set_setting("Stand Row Height",30)
    menu.set_setting("Stand Position X",191)
    menu.set_setting("Stand Position Y",43)
...
function draw_menu()
    ...
    local source_width=clamp(width_setting and width_setting.f_val or 476,120,1200)
    ...
    local position_x=clamp(position_x_setting and position_x_setting.f_val or 191,0,1920)*hud_sx
    local position_y=clamp(position_y_setting and position_y_setting.f_val or 43,0,1080)*hud_sy
        ...
        if item then
            ...
            if item.is_header then
                ...
                text.draw_ellipsis(font.item,text_x,yy+(row_h-text.height(font.item))*0.5,
                    active and 255 or 0,active and 255 or 0,active and 255 or 0,active and 255 or 255,
                ...
                if toggle_type then
                    ...
                    if toggle_handle and toggle_handle>0 then
                        ...
                        draw.rect_outline(indicator_x-5,yy+row_h*0.5-5,indicator_x+5,yy+row_h*0.5+5,
                            active and 255 or 0,active and 255 or 0,active and 255 or 0,210,1,1)
                ...
                elseif action_mark then
                    ...
                    if action_handle and action_handle>0 then
                        ...
                        text.draw(font.small,x+width-10-text.width(font.small,mark),yy+(row_h-text.height(font.small))*0.5,
                            active and 255 or 0,active and 255 or 0,active and 255 or 0,255,mark)
                    end
                end
                if value~="" then
