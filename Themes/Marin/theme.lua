...
local THEME_NAME="Marin"
local LAYOUT_MODE="standard"
local TAB_BOTTOM=false
local TABS_ENABLED=true
local HEADER_FILE="Themes\\Marin\\textures\\header.png"
local HEADER_PATH="Themes/Marin/textures/header.png"
...
local SETTINGS_FILE="stand_import_v3_marin.ini"
menu.clear_settings()
menu.add_setting_color("Stand Accent",255,119,218,255,"Imported primary colour")
menu.add_setting_color("Stand Background",255,229,255,255,"Imported background colour")
...
menu.add_setting_slider("Stand Position X",1479,0,1920,1,"Main-list X coordinate in Stand HUD pixels")
menu.add_setting_slider("Stand Position Y",337,0,1080,1,"Main-list Y coordinate in Stand HUD pixels")
...
local function load_settings()
    ...
    for line in body:gmatch("[^\r\n]+") do
        ...
        elseif key=="position_x" then menu.set_setting("Stand Position X",tonumber(value) or 1479)
        elseif key=="position_y" then menu.set_setting("Stand Position Y",tonumber(value) or 337)
        end
    end
end

local function serialize_settings()
    local accent=setting("Stand Accent") or {r=255,g=119,b=218,a=255}
    local background=setting("Stand Background") or {r=255,g=229,b=255,a=255}
    ...
    local position_x=setting("Stand Position X") or {f_val=1479}
    local position_y=setting("Stand Position Y") or {f_val=337}
    return table.concat({
...
local function reset_settings()
    menu.set_setting("Stand Accent",255,119,218,255)
    menu.set_setting("Stand Background",255,229,255,255)
    menu.set_setting("Stand Width",427)
    menu.set_setting("Stand Rows",13)
    menu.set_setting("Stand Row Height",30)
    menu.set_setting("Stand Position X",1479)
    menu.set_setting("Stand Position Y",337)
...
function draw_menu()
    ...
    local position_x=clamp(position_x_setting and position_x_setting.f_val or 1479,0,1920)*hud_sx
    local position_y=clamp(position_y_setting and position_y_setting.f_val or 337,0,1080)*hud_sy
    ...
    local page_h=SUBHEADER_PATH~="" and native_height(subheader_handle,width,25*hud_sy)
        or (TABS_ENABLED and 25*hud_sy or 0)
    ...
    if 255>0 and 4>0 then
        draw.rect_outline(x,content_y,x+width,content_y+body_h,255,255,255,255,0,4)
    end
    ...
    if page_h>0 then
        ...
        if TABS_ENABLED then
            ...
            text.draw(font.small,x+width-10-text.width(font.small,counter),
                page_y+(page_h-text.height(font.small))*0.5,31,22,47,255,counter)
        end
    end
        ...
        if item then
            ...
            if item.is_header then
                ...
                text.draw_ellipsis(font.item,text_x,yy+(row_h-text.height(font.item))*0.5,
                    active and 255 or 31,active and 255 or 22,active and 255 or 47,active and 255 or 255,
                ...
                if toggle_type then
                    ...
                    if toggle_handle and toggle_handle>0 then
                        ...
                        draw.rect_outline(indicator_x-5,yy+row_h*0.5-5,indicator_x+5,yy+row_h*0.5+5,
                            active and 255 or 31,active and 255 or 22,active and 255 or 47,210,1,1)
                ...
                elseif action_mark then
                    ...
                    if action_handle and action_handle>0 then
                        ...
                        text.draw(font.small,x+width-10-text.width(font.small,mark),yy+(row_h-text.height(font.small))*0.5,
                            active and 255 or 31,active and 255 or 22,active and 255 or 47,255,mark)
                    end
                end
                if value~="" then
