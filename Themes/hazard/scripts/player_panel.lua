-- Hazard-styled player-info panel. Overrides the global "player_panel" overlay when
-- the Hazard theme is active (theme scripts load after globals, override by name).
-- Black panel, caution-yellow borders/text, hazard caution-tape header, hard yellow
-- offset shadow, zero rounding, UPPERCASE labels. Shows full info + the live ped.

local PAPER  = { 15, 15, 15 }       -- black panel
local YELLOW = { 250, 204, 21 }     -- caution yellow (accent + ink)
local WHITE  = { 255, 255, 255 }
local GREEN  = { 163, 230, 53 }
local RED    = { 248, 113, 113 }
local DIM    = { 140, 130, 70 }
local BLUE   = { 125, 211, 252 }
...
local function hazard_tape(x, y, w, h)
    ...
    draw.rect(x, y, x + w, y + h, YELLOW[1], YELLOW[2], YELLOW[3], 255)
    local step = 22
    local sx = x - h
    while sx < x + w do
        draw.line(sx, y + h, sx + h, y, PAPER[1], PAPER[2], PAPER[3], 11)
        sx = sx + step
    end
    draw.pop_clip()
end
...
overlay.on_draw("player_panel", function()
    ...
    local colL = {
        { "row", "Name",        it.name or "Player", YELLOW },
    ...
    -- Hard yellow offset shadow + black body (ped slot left transparent)
    draw.rect(X0 + shadow, Y0 + shadow, X0 + W + shadow, Y0 + H + shadow, YELLOW[1], YELLOW[2], YELLOW[3], 255)
    ...
    -- Header: black with hazard tape across top, yellow title, thick yellow underline
    draw.rect(X0, Y0, X0 + W, Y0 + header_h, PAPER[1], PAPER[2], PAPER[3], 255)
    hazard_tape(X0, Y0, W, tape_h)
    text.draw(font.item, X0 + pad, Y0 + tape_h + (header_h - tape_h - text.height(font.item)) / 2, YELLOW[1], YELLOW[2], YELLOW[3], 255, string.upper(it.name or "PLAYER"))
    draw.rect(X0, Y0 + header_h - border, X0 + W, Y0 + header_h, YELLOW[1], YELLOW[2], YELLOW[3], 255)

    -- Thick yellow outer border + ped slot border
    draw.rect_outline(X0, Y0, X0 + W, Y0 + H, YELLOW[1], YELLOW[2], YELLOW[3], 255, 0, border)
    draw.rect_outline(px0, py0, px1, py1, YELLOW[1], YELLOW[2], YELLOW[3], 255, 0, 2)

    local ok = players.draw_ped(p.player_id or 0,
        px0 / sw, py0 / shh, (px1 - px0) / sw, (py1 - py0) / shh)
    if not ok then
        local s = "NO PREVIEW"
        text.draw(rfont, px0 + (LW - text.width(rfont, s)) / 2, py0 + ph_slot / 2 - 6, YELLOW[1], YELLOW[2], YELLOW[3], 150, s)
    end

    local function mini_bar(mbx, mby, mbw, frac, r, g, b)
        frac = clamp(frac, 0, 1)
        draw.rect(mbx, mby, mbx + mbw, mby + 6, YELLOW[1], YELLOW[2], YELLOW[3], 40)
        draw.rect(mbx, mby, mbx + mbw * frac, mby + 6, r, g, b, 255)
        draw.rect_outline(mbx, mby, mbx + mbw, mby + 6, YELLOW[1], YELLOW[2], YELLOW[3], 255, 0, 1)
    end
    ...
    local function render(cx, cw, yy, entries)
        for _, e in ipairs(entries) do
            if e[1] == "sec" then
                ...
                draw.rect(cx, yy, cx + lw + 8, yy + 14, YELLOW[1], YELLOW[2], YELLOW[3], 255)
                text.draw(font.label, cx + 4, yy + 2, PAPER[1], PAPER[2], PAPER[3], 255, up)
                draw.rect(cx, yy + 16, cx + cw, yy + 18, YELLOW[1], YELLOW[2], YELLOW[3], 255)
            elseif e[1] == "bar" then
                text.draw(rfont, cx, yy, YELLOW[1], YELLOW[2], YELLOW[3], 150, string.upper(e[2]))
