...
local SETTINGS_FILE="stand_import_v3_nordic_dark.ini"
menu.clear_settings()
menu.add_setting_color("Stand Accent",58,65,81,255,"Imported primary colour")
menu.add_setting_color("Stand Background",45,51,63,255,"Imported background colour")
...
menu.add_setting_slider("Stand Position X",1485,0,1920,1,"Main-list X coordinate in Stand HUD pixels")
menu.add_setting_slider("Stand Position Y",297,0,1080,1,"Main-list Y coordinate in Stand HUD pixels")
...
local function load_settings()
    ...
    for line in body:gmatch("[^\r\n]+") do
        ...
        elseif key=="position_x" then menu.set_setting("Stand Position X",tonumber(value) or 1485)
        elseif key=="position_y" then menu.set_setting("Stand Position Y",tonumber(value) or 297)
        end
    end
end

local function serialize_settings()
    local accent=setting("Stand Accent") or {r=58,g=65,b=81,a=255}
    local background=setting("Stand Background") or {r=45,g=51,b=63,a=255}
    ...
    local position_x=setting("Stand Position X") or {f_val=1485}
    local position_y=setting("Stand Position Y") or {f_val=297}
    return table.concat({
...
local function reset_settings()
    menu.set_setting("Stand Accent",58,65,81,255)
    menu.set_setting("Stand Background",45,51,63,255)
    menu.set_setting("Stand Width",406)
    menu.set_setting("Stand Rows",12)
    menu.set_setting("Stand Row Height",31)
    menu.set_setting("Stand Position X",1485)
    menu.set_setting("Stand Position Y",297)
...
function draw_menu()
    ...
    local position_x=clamp(position_x_setting and position_x_setting.f_val or 1485,0,1920)*hud_sx
    local position_y=clamp(position_y_setting and position_y_setting.f_val or 297,0,1080)*hud_sy
    ...
    if 255>0 and 4>0 then
        draw.rect_outline(x,content_y,x+width,content_y+body_h,46,52,64,255,5,4)
    end
    ...
    for row=0,visible-1 do
        ...
        if item then
            ...
            if 255>0 then
                draw.rect(x,yy+row_h-1,x+width,yy+row_h,46,52,64,math.min(255,80))
            end

            if item.is_header then
                text.draw(font.small,x+10,yy+(row_h-text.height(font.small))*0.5,
                    active and 83 or accent.r,active and 149 or accent.g,active and 224 or accent.b,255,
                ...
                text.draw_ellipsis(font.item,text_x,yy+(row_h-text.height(font.item))*0.5,
                    active and 83 or 238,active and 149 or 238,active and 224 or 242,active and 255 or 235,
                ...
                if toggle_type then
                    ...
                    if toggle_handle and toggle_handle>0 then
                        ...
                        draw.rect_outline(indicator_x-5,yy+row_h*0.5-5,indicator_x+5,yy+row_h*0.5+5,
                            active and 83 or 238,active and 149 or 238,active and 224 or 242,210,1,1)
                        if item.on then draw.rect(indicator_x-3,yy+row_h*0.5-3,indicator_x+3,yy+row_h*0.5+3,
                            active and 83 or accent.r,active and 149 or accent.g,active and 224 or accent.b,255,1) end
                    end
                ...
                elseif action_mark then
                    ...
                    if action_handle and action_handle>0 then
                        ...
                        text.draw(font.small,x+width-10-text.width(font.small,mark),yy+(row_h-text.height(font.small))*0.5,
                            active and 83 or 238,active and 149 or 238,active and 224 or 242,255,mark)
                    end
                end
                if value~="" then
                    ...
                    text.draw(font.value,right-text.width(font.value,value),yy+(row_h-text.height(font.value))*0.5,
                        active and 83 or 238,active and 149 or 238,active and 224 or 242,255,value)
                end
            end
