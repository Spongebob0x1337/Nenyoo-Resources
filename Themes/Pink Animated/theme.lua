...
local THEME_NAME="Pink Animated"
local LAYOUT_MODE="standard"
local TAB_BOTTOM=false
local TABS_ENABLED=true
local HEADER_FILE="Themes\\Pink Animated\\textures\\header.gif"
local HEADER_PATH="Themes/Pink Animated/textures/header.gif"
...
local SETTINGS_FILE="stand_import_v3_pink_animated.ini"
menu.clear_settings()
menu.add_setting_color("Stand Accent",240,73,179,255,"Imported primary colour")
menu.add_setting_color("Stand Background",10,10,10,255,"Imported background colour")
menu.add_setting_slider("Stand Width",426,120,1200,1,"Width in Stand's 1920x1080 HUD pixels")
...
local function load_settings()
    ...
    for line in body:gmatch("[^\r\n]+") do
        ...
        elseif key=="width" then menu.set_setting("Stand Width",tonumber(value) or 426)
...
local function serialize_settings()
    local accent=setting("Stand Accent") or {r=240,g=73,b=179,a=255}
    local background=setting("Stand Background") or {r=10,g=10,b=10,a=255}
    local width=setting("Stand Width") or {f_val=426}
...
local function reset_settings()
    menu.set_setting("Stand Accent",240,73,179,255)
    menu.set_setting("Stand Background",10,10,10,255)
    menu.set_setting("Stand Width",426)
...
function draw_menu()
    ...
    local source_width=clamp(width_setting and width_setting.f_val or 426,120,1200)
    ...
    if 255>0 and 3>0 then
        draw.rect_outline(x,content_y,x+width,content_y+body_h,10,10,10,255,5,3)
    end
    ...
    for row=0,visible-1 do
        ...
        if item then
            ...
            if 255>0 then
                draw.rect(x,yy+row_h-1,x+width,yy+row_h,10,10,10,math.min(255,80))
            end
