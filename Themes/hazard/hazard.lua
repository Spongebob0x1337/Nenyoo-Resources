-- Nenyoo "Hazard" Theme — yellow & black neo-brutalist
-- Black panel, caution-yellow borders/text, a hazard caution-tape header
-- strip, hard yellow offset shadows, zero rounding, chunky [ ON ]/[ OFF ]
-- toggles, and a solid-yellow selected bar with black text. THEME_API.md.
...
local function reload_colors()
    return {
        accent  = sc("Hazard Yellow") or {250, 204, 21, 255},  -- caution yellow
        accent2 = sc("Secondary")     or {255, 255, 255, 255}, -- white
        paper   = sc("Panel")         or {15, 15, 15, 255},    -- black panel
        ink     = sc("Yellow Ink")    or {250, 204, 21, 255},  -- yellow text/borders
    }
end
local function reload_layout()
    return {
        ...
        header_h = 72, sub_h = 30, desc_h = 56,
        list_max = 440, pad_x = 16,
        border = 3, shadow = 7, tape_h = 12,
    }
end

menu.clear_settings()
menu.add_setting_submenu("Colors", "Hazard palette")
menu.add_sub_color("Hazard Yellow", 250, 204, 21, 255, "Accent / highlight")
menu.add_sub_color("Secondary", 255, 255, 255, 255, "Secondary accent")
menu.add_sub_color("Panel", 15, 15, 15, 255, "Panel background")
menu.add_sub_color("Yellow Ink", 250, 204, 21, 255, "Text / borders")
...
-- text flips to black when sitting on a solid yellow highlight
local function ink(is_sel)
    if is_sel then return c.paper[1], c.paper[2], c.paper[3] end
    return c.ink[1], c.ink[2], c.ink[3]
end

-- hard-shadowed, yellow-bordered box on black
local function block(x1, y1, x2, y2, fr, fg, fb, fa)
    draw.rect(x1+l.shadow, y1+l.shadow, x2+l.shadow, y2+l.shadow, c.accent[1], c.accent[2], c.accent[3], 255) -- yellow shadow
    draw.rect(x1, y1, x2, y2, fr, fg, fb, fa or 255)
    draw.rect_outline(x1, y1, x2, y2, c.ink[1], c.ink[2], c.ink[3], 255, 0, l.border)
end

-- diagonal hazard caution-tape strip
local function hazard_tape(x, y, w, h)
    draw.push_clip(x, y, x + w, y + h)
    draw.rect(x, y, x + w, y + h, c.accent[1], c.accent[2], c.accent[3], 255)
    local step = 22
    local sx = x - h
    while sx < x + w do
        draw.line(sx, y + h, sx + h, y, c.paper[1], c.paper[2], c.paper[3], 11)
        sx = sx + step
    end
    draw.pop_clip()
end
...
-- Widgets
---------------------------------------------------------------------------
local function draw_toggle(rx, yc, item)
    -- self-contained contrast: works on both black rows and the yellow bar
    local on = item.on
    local txt = on and "ON" or "OFF"
    local tw = math.max(text.width(font.value, "OFF") + 20, 46)
    local th = 22
    local tx, ty = rx - tw, yc - th/2
    local fill   = on and c.accent or c.paper
    local border = on and c.paper  or c.accent
    local tc     = on and c.paper  or c.accent
    draw.rect(tx, ty, tx+tw, ty+th, fill[1], fill[2], fill[3], 255)
    draw.rect_outline(tx, ty, tx+tw, ty+th, border[1], border[2], border[3], 255, 0, 2)
...
local function draw_slider(rx, yc, item, is_sel)
    ...
    if fw > 0 then draw.rect(sx, sy, sx+fw, sy+12, ir, ig, ib, 255) end
...
local function draw_stepper(rx, yc, val, is_sel)
    local ir, ig, ib = ink(is_sel)
    local txt = "[ " .. val .. " ]"
    text.draw(font.value, rx-text.width(font.value, txt), yc-text.height(font.value)/2, ir, ig, ib, 255, txt)
end
...
local function draw_widget(rx, yc, item, idx, is_sel)
    ...
    elseif t == item_type.selected_tick then
        text.draw(font.item, rx-text.width(font.item, "[v]"), yc-text.height(font.item)/2, ir, ig, ib, 255, "[v]")
    elseif t == item_type.toggle then draw_toggle(rx, yc, item)
    elseif t == item_type.float_toggle then local w=draw_toggle(rx, yc, item); draw_slider(rx-w-GAP, yc, item, is_sel)
    elseif t == item_type.int_toggle then local w=draw_toggle(rx, yc, item); draw_stepper(rx-w-GAP, yc, tostring(item.i_val), is_sel)
    elseif t == item_type.array_toggle or t == item_type.loop_toggle then local w=draw_toggle(rx, yc, item); draw_stepper(rx-w-GAP, yc, item.current_value or "?", is_sel)
    ...
    elseif t == item_type.search then
        -- magnifier glyph on the right, current query (or dim placeholder) to its left
        local glyph = "[Q]"
        local gw = text.width(font.value, glyph)
        text.draw(font.value, rx-gw, yc-text.height(font.value)/2, ir, ig, ib, 255, glyph)
        local q = (item.text and item.text ~= "") and item.text or "Search..."
        local qa = (item.text and item.text ~= "") and 255 or 120
        local qw = text.width(font.value, q)
        text.draw(font.value, rx-gw-8-qw, yc-text.height(font.value)/2, ir, ig, ib, qa, q)
    elseif t == item_type.input_text then
        text.draw(font.value, rx-text.width(font.value,item.name), yc-text.height(font.value)/2, ir, ig, ib, 255, item.name)
...
local function draw_hotkey_badge(x, y, vk, is_sel, lh)
    ...
    -- yellow chip with black text (reads on both row states)
    draw.rect(x-4, by-1, x+bw+4, by+bh+2, c.accent[1], c.accent[2], c.accent[3], 255)
    draw.rect_outline(x-4, by-1, x+bw+4, by+bh+2, c.paper[1],c.paper[2],c.paper[3], 255, 0, 1)
    text.draw(font.tiny, x, by, c.paper[1],c.paper[2],c.paper[3], 255, kn)
end
...
local function draw_header(x, y)
    -- black header, hazard tape across the top, yellow title
    draw.rect(x, y, x+l.menu_w, y+l.header_h, c.paper[1], c.paper[2], c.paper[3], 255)
    hazard_tape(x, y, l.menu_w, l.tape_h)
    local title = string.upper(str.brand or "NENYOO")
    text.draw_spaced(font.title, x+l.pad_x, y+l.tape_h+(l.header_h-l.tape_h-text.height(font.title))/2, c.ink[1],c.ink[2],c.ink[3],255, title, 2)
    -- caution triangle mark on the right
    local cxr = x+l.menu_w-l.pad_x-26
    local cyr = y+l.tape_h+(l.header_h-l.tape_h)/2
    draw.line(cxr, cyr-12, cxr+13, cyr+10, c.accent[1],c.accent[2],c.accent[3], 3)
    draw.line(cxr+26, cyr-12, cxr+13, cyr+10, c.accent[1],c.accent[2],c.accent[3], 3)
    draw.line(cxr, cyr-12, cxr+26, cyr-12, c.accent[1],c.accent[2],c.accent[3], 3)
    text.draw(font.value, cxr+10, cyr-8, c.accent[1],c.accent[2],c.accent[3],255, "!")
    -- thick bottom border
    draw.rect(x, y+l.header_h-l.border, x+l.menu_w, y+l.header_h, c.ink[1], c.ink[2], c.ink[3], 255)
end

local function draw_subtitle(x, y)
    draw.rect(x, y, x+l.menu_w, y+l.sub_h, c.paper[1], c.paper[2], c.paper[3], 255)
    local page = string.upper(menu.page_name())
    text.draw_spaced(font.breadcrumb, x+l.pad_x, y+(l.sub_h-text.height(font.breadcrumb))/2, c.ink[1],c.ink[2],c.ink[3], 255, page, 1)
    local counter = string.format("%02d/%02d", menu.selected_index()+1, menu.item_count())
    local cw = text.width(font.breadcrumb, counter)
    text.draw(font.breadcrumb, x+l.menu_w-l.pad_x-cw, y+(l.sub_h-text.height(font.breadcrumb))/2, c.accent2[1],c.accent2[2],c.accent2[3], 200, counter)
    draw.rect(x, y+l.sub_h-2, x+l.menu_w, y+l.sub_h, c.ink[1],c.ink[2],c.ink[3], 255)
end
...
local function draw_scrollbar(x, list_y, list_h, content_h)
    ...
    draw.rect(sx, list_y, sx+sw, list_y+list_h, c.ink[1],c.ink[2],c.ink[3], 30)
    draw.rect(sx, thumb_y, sx+sw, thumb_y+thumb_h, c.accent[1], c.accent[2], c.accent[3], 255)
    draw.rect_outline(sx, thumb_y, sx+sw, thumb_y+thumb_h, c.paper[1],c.paper[2],c.paper[3], 255, 0, 2)
end
...
local function draw_side_panel(x, y, w, item)
    ...
    local green,red,yellow,blue = {163,230,53},{248,113,113},c.accent,{125,211,252}
    local valc = {c.ink[1],c.ink[2],c.ink[3],255}
    local cy = y
    local function section(t2)
        cy = cy+6
        local lw = text.width(font.label,string.upper(t2))
        draw.rect(x+12, cy, x+12+lw+8, cy+14, c.accent[1],c.accent[2],c.accent[3], 255)
        text.draw(font.label, x+16, cy+3, c.paper[1],c.paper[2],c.paper[3], 255, string.upper(t2)); cy=cy+18
    ...
    local function mini_bar(bx,by,bw,frac,col)
        draw.rect(bx,by,bx+bw,by+6,c.ink[1],c.ink[2],c.ink[3],40); draw.rect(bx,by,bx+bw*clamp(frac,0,1),by+6,col[1],col[2],col[3],255)
    ...
    local function image_box(label)
        draw.rect(x+12, cy, x+w-12, cy+76, c.ink[1],c.ink[2],c.ink[3],20)
    ...
    if it==1 then
        ...
        text.draw(font.small, x+14, cy+4, red[1],red[2],red[3],220, str.health or "HP")
        mini_bar(x+72, cy+7, 56, info.health/100, info.health>50 and green or red)
        text.draw(font.tiny, x+134, cy+4, c.ink[1],c.ink[2],c.ink[3],220, tostring(info.health).."%")
        ...
        text.draw(font.tiny, x+w*0.5+118, cy+4, c.ink[1],c.ink[2],c.ink[3],220, tostring(info.armor).."%"); cy=cy+22
...
function draw_menu()
    ...
    -- yellow hard shadow + black panel
    draw.rect(mx+l.shadow, my+l.shadow, mx+l.menu_w+l.shadow, my+total_h+l.shadow, c.accent[1], c.accent[2], c.accent[3], 255)
    ...
    for i = 0, count-1 do
        ...
        if is_sel then
            ...
            -- solid yellow highlight, black text
            draw.rect(mx+4, iy+2, ibx-4, iy+l.item_h-2, c.accent[1], c.accent[2], c.accent[3], math.floor(255*at))
            draw.rect(mx+4, iy+2, mx+10, iy+l.item_h-2, c.paper[1], c.paper[2], c.paper[3], math.floor(255*at))
        elseif hovered then
            draw.rect(mx+4, iy+2, ibx-4, iy+l.item_h-2, c.accent[1], c.accent[2], c.accent[3], math.floor(34*at))
        end
...
function draw_input_popup()
    ...
    draw.rect(ix, iy, ix+iw, iy+ih, c.ink[1],c.ink[2],c.ink[3], 16)
    ...
    if math.fmod(ctx.time()-input_blink, 1.0) < 0.55 then
        ...
        draw.rect(tx+cw, iy+7, tx+cw+2, iy+ih-7, c.accent[1],c.accent[2],c.accent[3], 255)
    end
end
...
menu.open_input_popup = function()
    local it = menu.get_item(menu.input_target_item and menu.input_target_item() or menu.selected_index())
    if it and it.type==item_type.search then menu.set_input_buffer(it.text or "") end
    orig_open_input(); input_cursor=#menu.get_input_buffer(); input_blink=ctx.time(); input_open_time=ctx.time()
end
...
function draw_array_dropdown_popup()
    ...
    for i = 1, count do
        ...
        if is_sel or hovered then draw.rect(dx+3, iy, dx+dw-3, iy+dh_item, c.accent[1],c.accent[2],c.accent[3], 255); ir,ig,ib = c.paper[1],c.paper[2],c.paper[3] end
        text.draw(font.value, dx+12, iy+(dh_item-vfh)/2, ir,ig,ib, 255, string.upper(values[i]))
        if is_cur then text.draw(font.value, dx+dw-22, iy+(dh_item-vfh)/2, ir,ig,ib, 255, "*") end
    end
end
...
function draw_hotkey_popup()
    ...
    local px, py = (sw-pw)/2, (sh-ph)/2
    block(px, py, px+pw, py+ph, c.paper[1],c.paper[2],c.paper[3],255)
    hazard_tape(px, py, pw, 8)
    local title = string.upper(str.press_key or "PRESS ANY KEY")
    text.draw(font.item, px+(pw-text.width(font.item,title))/2, py+22, c.ink[1],c.ink[2],c.ink[3],255, title)
    local hint = str.esc_cancel or "ESC TO CANCEL"
    text.draw(font.desc, px+(pw-text.width(font.desc,hint))/2, py+22+text.height(font.item)+8, c.ink[1],c.ink[2],c.ink[3],170, hint)
end
...
function draw_color_picker()
    ...
    for i=1,2 do
        ...
        local tc = active and {c.paper[1],c.paper[2],c.paper[3]} or {c.ink[1],c.ink[2],c.ink[3]}
    ...
    if cp_tab==0 then
        ...
        for i=1,32 do
            ...
            draw.rect_outline(cx, cyy, cx+cell, cyy+cell, c.ink[1],c.ink[2],c.ink[3], 200, 0, 1)
