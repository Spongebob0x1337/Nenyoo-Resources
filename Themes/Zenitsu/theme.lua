...
local THEME_NAME="Zenitsu"
local LAYOUT_MODE="standard"
local TAB_BOTTOM=false
local TABS_ENABLED=true
local HEADER_FILE="Themes\\Zenitsu\\textures\\header.png"
local HEADER_PATH="Themes/Zenitsu/textures/header.png"
...
local SETTINGS_FILE="stand_import_v3_zenitsu.ini"
menu.clear_settings()
menu.add_setting_color("Stand Accent",255,162,55,255,"Imported primary colour")
...
menu.add_setting_slider("Stand Position X",1462,0,1920,1,"Main-list X coordinate in Stand HUD pixels")
menu.add_setting_slider("Stand Position Y",560,0,1080,1,"Main-list Y coordinate in Stand HUD pixels")
...
local function load_settings()
    ...
    for line in body:gmatch("[^\r\n]+") do
        ...
        elseif key=="position_x" then menu.set_setting("Stand Position X",tonumber(value) or 1462)
        elseif key=="position_y" then menu.set_setting("Stand Position Y",tonumber(value) or 560)
        end
    end
end

local function serialize_settings()
    local accent=setting("Stand Accent") or {r=255,g=162,b=55,a=255}
    ...
    local position_x=setting("Stand Position X") or {f_val=1462}
    local position_y=setting("Stand Position Y") or {f_val=560}
    return table.concat({
...
local function reset_settings()
    menu.set_setting("Stand Accent",255,162,55,255)
    ...
    menu.set_setting("Stand Position X",1462)
    menu.set_setting("Stand Position Y",560)
...
function draw_menu()
    ...
    local position_x=clamp(position_x_setting and position_x_setting.f_val or 1462,0,1920)*hud_sx
    local position_y=clamp(position_y_setting and position_y_setting.f_val or 560,0,1080)*hud_sy
    ...
    if 255>0 and 2>0 then
        draw.rect_outline(x,content_y,x+width,content_y+body_h,181,116,54,255,5,2)
    end
    ...
    for row=0,visible-1 do
        ...
        if item then
            ...
            if 255>0 then
                draw.rect(x,yy+row_h-1,x+width,yy+row_h,181,116,54,math.min(255,80))
            end
