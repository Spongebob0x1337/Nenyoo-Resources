...
local THEME_NAME="Kokushibo"
local LAYOUT_MODE="standard"
local TAB_BOTTOM=false
local TABS_ENABLED=false
local HEADER_FILE="Themes\\Kokushibo\\textures\\header.png"
local HEADER_PATH="Themes/Kokushibo/textures/header.png"
...
local SETTINGS_FILE="stand_import_v3_kokushibo.ini"
menu.clear_settings()
menu.add_setting_color("Stand Accent",54,44,71,255,"Imported primary colour")
menu.add_setting_color("Stand Background",0,0,0,100,"Imported background colour")
menu.add_setting_slider("Stand Width",442,120,1200,1,"Width in Stand's 1920x1080 HUD pixels")
...
menu.add_setting_slider("Stand Position X",1468,0,1920,1,"Main-list X coordinate in Stand HUD pixels")
menu.add_setting_slider("Stand Position Y",311,0,1080,1,"Main-list Y coordinate in Stand HUD pixels")
...
local function load_settings()
    ...
    for line in body:gmatch("[^\r\n]+") do
        ...
        elseif key=="width" then menu.set_setting("Stand Width",tonumber(value) or 442)
        ...
        elseif key=="position_x" then menu.set_setting("Stand Position X",tonumber(value) or 1468)
        elseif key=="position_y" then menu.set_setting("Stand Position Y",tonumber(value) or 311)
        end
    end
end

local function serialize_settings()
    local accent=setting("Stand Accent") or {r=54,g=44,b=71,a=255}
    local background=setting("Stand Background") or {r=0,g=0,b=0,a=100}
    local width=setting("Stand Width") or {f_val=442}
    local rows=setting("Stand Rows") or {f_val=12}
    local row_height=setting("Stand Row Height") or {f_val=31}
    local position_x=setting("Stand Position X") or {f_val=1468}
    local position_y=setting("Stand Position Y") or {f_val=311}
    return table.concat({
...
local function reset_settings()
    menu.set_setting("Stand Accent",54,44,71,255)
    menu.set_setting("Stand Background",0,0,0,100)
    menu.set_setting("Stand Width",442)
    menu.set_setting("Stand Rows",12)
    menu.set_setting("Stand Row Height",31)
    menu.set_setting("Stand Position X",1468)
    menu.set_setting("Stand Position Y",311)
...
function draw_menu()
    ...
    local source_width=clamp(width_setting and width_setting.f_val or 442,120,1200)
    ...
    local position_x=clamp(position_x_setting and position_x_setting.f_val or 1468,0,1920)*hud_sx
    local position_y=clamp(position_y_setting and position_y_setting.f_val or 311,0,1080)*hud_sy
    ...
    if 255>0 and 3>0 then
        draw.rect_outline(x,content_y,x+width,content_y+body_h,15,16,18,255,5,3)
    end
    ...
    for row=0,visible-1 do
        ...
        if item then
            ...
            if 255>0 then
                draw.rect(x,yy+row_h-1,x+width,yy+row_h,15,16,18,math.min(255,80))
            end
