...
local THEME_NAME="Rebound"
local LAYOUT_MODE="standard"
local TAB_BOTTOM=false
local TABS_ENABLED=false
local HEADER_FILE="Themes\\Rebound\\textures\\header.bmp"
local HEADER_PATH="Themes/Rebound/textures/header.bmp"
...
local SUBHEADER_FILE="Themes\\Rebound\\textures\\subheader.bmp"
local SUBHEADER_PATH="Themes/Rebound/textures/subheader.bmp"
local SUBHEADER_URL_PATH=""
local subheader_handle=0
local subheader_downloaded=false
local FOOTER_FILE="Themes\\Rebound\\textures\\footer.bmp"
local FOOTER_PATH="Themes/Rebound/textures/footer.bmp"
...
local TOGGLE_ON_FILE="Themes\\Rebound\\textures\\toggle_on.png"
local TOGGLE_ON_PATH="Themes/Rebound/textures/toggle_on.png"
local TOGGLE_ON_URL_PATH=""
local toggle_on_handle=0
local toggle_on_downloaded=false
local TOGGLE_OFF_FILE="Themes\\Rebound\\textures\\toggle_off.png"
local TOGGLE_OFF_PATH="Themes/Rebound/textures/toggle_off.png"
local TOGGLE_OFF_URL_PATH=""
local toggle_off_handle=0
local toggle_off_downloaded=false
local LIST_ICON_FILE="Themes\\Rebound\\textures\\list_icon.png"
local LIST_ICON_PATH="Themes/Rebound/textures/list_icon.png"
local LIST_ICON_URL_PATH=""
local list_icon_handle=0
local list_icon_downloaded=false
local UI_ASSETS={
    ["list"]={file="Themes\\Rebound\\textures\\ui_theme_list.png",path="Themes/Rebound/textures/ui_theme_list.png",url=""},
    ["toggle off"]={file="Themes\\Rebound\\textures\\ui_theme_toggle_off.png",path="Themes/Rebound/textures/ui_theme_toggle_off.png",url=""},
    ["toggle on"]={file="Themes\\Rebound\\textures\\ui_theme_toggle_on.png",path="Themes/Rebound/textures/ui_theme_toggle_on.png",url=""},
}
...
local SETTINGS_FILE="stand_import_v3_rebound.ini"
menu.clear_settings()
menu.add_setting_color("Stand Accent",255,117,58,255,"Imported primary colour")
menu.add_setting_color("Stand Background",0,0,0,150,"Imported background colour")
menu.add_setting_slider("Stand Width",460,120,1200,1,"Width in Stand's 1920x1080 HUD pixels")
...
menu.add_setting_slider("Stand Position X",150,0,1920,1,"Main-list X coordinate in Stand HUD pixels")
menu.add_setting_slider("Stand Position Y",230,0,1080,1,"Main-list Y coordinate in Stand HUD pixels")
...
local function load_settings()
    ...
    for line in body:gmatch("[^\r\n]+") do
        ...
        elseif key=="width" then menu.set_setting("Stand Width",tonumber(value) or 460)
        ...
        elseif key=="position_x" then menu.set_setting("Stand Position X",tonumber(value) or 150)
        elseif key=="position_y" then menu.set_setting("Stand Position Y",tonumber(value) or 230)
        end
    end
end

local function serialize_settings()
    local accent=setting("Stand Accent") or {r=255,g=117,b=58,a=255}
    local background=setting("Stand Background") or {r=0,g=0,b=0,a=150}
    local width=setting("Stand Width") or {f_val=460}
    local rows=setting("Stand Rows") or {f_val=12}
    local row_height=setting("Stand Row Height") or {f_val=35}
    local position_x=setting("Stand Position X") or {f_val=150}
    local position_y=setting("Stand Position Y") or {f_val=230}
    return table.concat({
...
local function reset_settings()
    menu.set_setting("Stand Accent",255,117,58,255)
    menu.set_setting("Stand Background",0,0,0,150)
    menu.set_setting("Stand Width",460)
    menu.set_setting("Stand Rows",12)
    menu.set_setting("Stand Row Height",35)
    menu.set_setting("Stand Position X",150)
    menu.set_setting("Stand Position Y",230)
...
function draw_menu()
    ...
    local source_width=clamp(width_setting and width_setting.f_val or 460,120,1200)
    ...
    local position_x=clamp(position_x_setting and position_x_setting.f_val or 150,0,1920)*hud_sx
    local position_y=clamp(position_y_setting and position_y_setting.f_val or 230,0,1080)*hud_sy
