...
local THEME_NAME="Rockstar Admin Tool"
local LAYOUT_MODE="standard"
local TAB_BOTTOM=false
local TABS_ENABLED=false
local HEADER_FILE="Themes\\Rockstar Admin Tool\\textures\\header.gif"
local HEADER_PATH="Themes/Rockstar Admin Tool/textures/header.gif"
...
local SETTINGS_FILE="stand_import_v3_rockstar_admin_tool.ini"
menu.clear_settings()
menu.add_setting_color("Stand Accent",7,7,7,255,"Imported primary colour")
menu.add_setting_color("Stand Background",10,10,10,200,"Imported background colour")
...
menu.add_setting_slider("Stand Position X",1420,0,1920,1,"Main-list X coordinate in Stand HUD pixels")
...
local function load_settings()
    ...
    for line in body:gmatch("[^\r\n]+") do
        ...
        elseif key=="position_x" then menu.set_setting("Stand Position X",tonumber(value) or 1420)
...
local function serialize_settings()
    local accent=setting("Stand Accent") or {r=7,g=7,b=7,a=255}
    local background=setting("Stand Background") or {r=10,g=10,b=10,a=200}
    ...
    local position_x=setting("Stand Position X") or {f_val=1420}
...
local function reset_settings()
    menu.set_setting("Stand Accent",7,7,7,255)
    menu.set_setting("Stand Background",10,10,10,200)
    menu.set_setting("Stand Width",462)
    menu.set_setting("Stand Rows",15)
    menu.set_setting("Stand Row Height",35)
    menu.set_setting("Stand Position X",1420)
...
function draw_menu()
    ...
    local position_x=clamp(position_x_setting and position_x_setting.f_val or 1420,0,1920)*hud_sx
