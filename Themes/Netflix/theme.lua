...
local THEME_NAME="Netflix"
local LAYOUT_MODE="standard"
local TAB_BOTTOM=false
local TABS_ENABLED=true
local HEADER_FILE="Themes\\Netflix\\textures\\header.gif"
local HEADER_PATH="Themes/Netflix/textures/header.gif"
...
local SETTINGS_FILE="stand_import_v3_netflix.ini"
menu.clear_settings()
...
function draw_menu()
    ...
    if 255>0 and 2>0 then
        draw.rect_outline(x,content_y,x+width,content_y+body_h,255,0,0,255,0,2)
    end
    ...
    for row=0,visible-1 do
        ...
        if item then
            ...
            if 255>0 then
                draw.rect(x,yy+row_h-1,x+width,yy+row_h,255,0,0,math.min(255,80))
            end
