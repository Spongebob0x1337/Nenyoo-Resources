...
local THEME_NAME="Blue Hour"
local LAYOUT_MODE="standard"
local TAB_BOTTOM=false
local TABS_ENABLED=true
local HEADER_FILE="Themes\\Blue Hour\\textures\\header.gif"
local HEADER_PATH="Themes/Blue Hour/textures/header.gif"
...
local SETTINGS_FILE="stand_import_v3_blue_hour.ini"
menu.clear_settings()
menu.add_setting_color("Stand Accent",85,135,82,255,"Imported primary colour")
menu.add_setting_color("Stand Background",26,26,43,200,"Imported background colour")
...
menu.add_setting_slider("Stand Position X",1427,0,1920,1,"Main-list X coordinate in Stand HUD pixels")
menu.add_setting_slider("Stand Position Y",153,0,1080,1,"Main-list Y coordinate in Stand HUD pixels")
...
local function load_settings()
    ...
    for line in body:gmatch("[^\r\n]+") do
        ...
        elseif key=="position_x" then menu.set_setting("Stand Position X",tonumber(value) or 1427)
        elseif key=="position_y" then menu.set_setting("Stand Position Y",tonumber(value) or 153)
        end
    end
end

local function serialize_settings()
    local accent=setting("Stand Accent") or {r=85,g=135,b=82,a=255}
    local background=setting("Stand Background") or {r=26,g=26,b=43,a=200}
    ...
    local position_x=setting("Stand Position X") or {f_val=1427}
    local position_y=setting("Stand Position Y") or {f_val=153}
    return table.concat({
...
local function reset_settings()
    menu.set_setting("Stand Accent",85,135,82,255)
    menu.set_setting("Stand Background",26,26,43,200)
    menu.set_setting("Stand Width",350)
    menu.set_setting("Stand Rows",12)
    menu.set_setting("Stand Row Height",30)
    menu.set_setting("Stand Position X",1427)
    menu.set_setting("Stand Position Y",153)
...
function draw_menu()
    ...
    local position_x=clamp(position_x_setting and position_x_setting.f_val or 1427,0,1920)*hud_sx
    local position_y=clamp(position_y_setting and position_y_setting.f_val or 153,0,1080)*hud_sy
        ...
        if item then
            ...
            if item.is_header then
                text.draw(font.small,x+10,yy+(row_h-text.height(font.small))*0.5,
                    active and 158 or accent.r,active and 255 or accent.g,active and 211 or accent.b,255,
                ...
                text.draw_ellipsis(font.item,text_x,yy+(row_h-text.height(font.item))*0.5,
                    active and 158 or 238,active and 255 or 238,active and 211 or 242,active and 255 or 235,
                ...
                if toggle_type then
                    ...
                    if toggle_handle and toggle_handle>0 then
                        ...
                        draw.rect_outline(indicator_x-5,yy+row_h*0.5-5,indicator_x+5,yy+row_h*0.5+5,
                            active and 158 or 238,active and 255 or 238,active and 211 or 242,210,1,1)
                        if item.on then draw.rect(indicator_x-3,yy+row_h*0.5-3,indicator_x+3,yy+row_h*0.5+3,
                            active and 158 or accent.r,active and 255 or accent.g,active and 211 or accent.b,255,1) end
                    end
                ...
                elseif action_mark then
                    ...
                    if action_handle and action_handle>0 then
                        ...
                        text.draw(font.small,x+width-10-text.width(font.small,mark),yy+(row_h-text.height(font.small))*0.5,
                            active and 158 or 238,active and 255 or 238,active and 211 or 242,255,mark)
                    end
                end
                if value~="" then
                    ...
                    text.draw(font.value,right-text.width(font.value,value),yy+(row_h-text.height(font.value))*0.5,
                        active and 158 or 238,active and 255 or 238,active and 211 or 242,255,value)
                end
            end
