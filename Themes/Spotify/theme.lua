...
local THEME_NAME="Spotify"
local LAYOUT_MODE="standard"
local TAB_BOTTOM=false
local TABS_ENABLED=true
local HEADER_FILE="Themes\\Spotify\\textures\\header.gif"
local HEADER_PATH="Themes/Spotify/textures/header.gif"
...
local SETTINGS_FILE="stand_import_v3_spotify.ini"
menu.clear_settings()
menu.add_setting_color("Stand Accent",0,150,0,0,"Imported primary colour")
menu.add_setting_color("Stand Background",0,0,0,255,"Imported background colour")
menu.add_setting_slider("Stand Width",423,120,1200,1,"Width in Stand's 1920x1080 HUD pixels")
...
menu.add_setting_slider("Stand Position X",1339,0,1920,1,"Main-list X coordinate in Stand HUD pixels")
menu.add_setting_slider("Stand Position Y",385,0,1080,1,"Main-list Y coordinate in Stand HUD pixels")
...
local function load_settings()
    ...
    for line in body:gmatch("[^\r\n]+") do
        ...
        elseif key=="width" then menu.set_setting("Stand Width",tonumber(value) or 423)
        ...
        elseif key=="position_x" then menu.set_setting("Stand Position X",tonumber(value) or 1339)
        elseif key=="position_y" then menu.set_setting("Stand Position Y",tonumber(value) or 385)
        end
    end
end

local function serialize_settings()
    local accent=setting("Stand Accent") or {r=0,g=150,b=0,a=0}
    local background=setting("Stand Background") or {r=0,g=0,b=0,a=255}
    local width=setting("Stand Width") or {f_val=423}
    local rows=setting("Stand Rows") or {f_val=13}
    local row_height=setting("Stand Row Height") or {f_val=31}
    local position_x=setting("Stand Position X") or {f_val=1339}
    local position_y=setting("Stand Position Y") or {f_val=385}
    return table.concat({
...
local function reset_settings()
    menu.set_setting("Stand Accent",0,150,0,0)
    menu.set_setting("Stand Background",0,0,0,255)
    menu.set_setting("Stand Width",423)
    menu.set_setting("Stand Rows",13)
    menu.set_setting("Stand Row Height",31)
    menu.set_setting("Stand Position X",1339)
    menu.set_setting("Stand Position Y",385)
...
function draw_menu()
    ...
    local source_width=clamp(width_setting and width_setting.f_val or 423,120,1200)
    ...
    local position_x=clamp(position_x_setting and position_x_setting.f_val or 1339,0,1920)*hud_sx
    local position_y=clamp(position_y_setting and position_y_setting.f_val or 385,0,1080)*hud_sy
    ...
    if 255>0 and 2>0 then
        draw.rect_outline(x,content_y,x+width,content_y+body_h,0,185,0,255,0,2)
    end
    ...
    for row=0,visible-1 do
        ...
        if item then
            ...
            if 255>0 then
                draw.rect(x,yy+row_h-1,x+width,yy+row_h,0,185,0,math.min(255,80))
            end
