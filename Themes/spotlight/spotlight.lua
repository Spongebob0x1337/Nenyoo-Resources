-- Nenyoo "Spotlight" Theme — command-palette / launcher style
-- A floating rounded panel near the top with a search-bar header (magnifier
-- + live query), an airy list with a soft accent selection fill, and a
-- bottom action bar. Built on THEME_API.md.

text.set_size(font.title, 18);  text.set_weight(font.title, 600)
text.set_size(font.item, 14);   text.set_weight(font.item, 500)
text.set_size(font.value, 13);  text.set_weight(font.value, 500)
...
local function reload_colors()
    return {
        accent  = sc("Accent")  or {110, 120, 255, 255},  -- indigo
        accentL = {150, 160, 255, 255},
        bg      = sc("Background") or {20, 21, 27, 255},
        surface = sc("Surface")    or {30, 32, 40, 255},
        bar     = {16, 17, 22, 255},
    }
end
local function reload_layout()
    return {
        menu_w = sf("Menu Width", 480),
        item_h = sf("Row Height", 40),
        search_h = 54, foot_h = 34,
        list_max = 420, pad_x = 16, round = 16,
        top_frac = sf("Top Offset", 14) / 100,
    }
end

menu.clear_settings()
menu.add_setting_submenu("Colors", "Accent and surfaces")
menu.add_sub_color("Accent", 110, 120, 255, 255, "Selection / highlights")
menu.add_sub_color("Background", 20, 21, 27, 255, "Panel background")
menu.add_sub_color("Surface", 30, 32, 40, 255, "Controls / chips")
menu.add_setting_submenu("Layout", "Sizes")
menu.add_sub_slider("Menu Width", 480, 360, 720, 10, "Panel width (px)")
menu.add_sub_slider("Row Height", 40, 30, 60, 1, "Row height (px)")
menu.add_sub_slider("Top Offset", 14, 4, 40, 1, "Distance from top (%)")
...
local function draw_toggle(rx, yc, item, idx)
    ...
    local off = {64,66,78}
    local tr = math.floor(lerp(off[1], c.accent[1], anim))
    local tg = math.floor(lerp(off[2], c.accent[2], anim))
    local tb = math.floor(lerp(off[3], c.accent[3], anim))
...
local function draw_slider(rx, yc, item, is_sel)
    ...
    draw.rect(sx, sy, sx+SLIDER_W, sy+4, 255,255,255, is_sel and 28 or 16, 2)
    ...
    text.draw(font.value, sx-10-text.width(font.value, vs), yc-text.height(font.value)/2, 255,255,255, is_sel and 230 or 150, vs)
    return sx, SLIDER_W
end
...
local function draw_stepper(rx, yc, val, is_sel)
    ...
    local ry = yc - lh/2
    local aa, va = is_sel and 200 or 120, is_sel and 235 or 170
    local rgt, lft = text.width(af, "›"), text.width(af, "‹")
    -- use ASCII to be safe
    rgt, lft = text.width(af, ">"), text.width(af, "<")
    local vw = text.width(af, val)
    text.draw(af, rx-rgt, ry, 255,255,255, aa, ">")
    local vx = rx-rgt-10-vw
    text.draw(af, vx, ry, 255,255,255, va, val)
    text.draw(af, vx-10-lft, ry, 255,255,255, aa, "<")
end

local function draw_widget(rx, yc, item, idx, is_sel)
    local t = item.type
    if t == item_type.sub_menu then
        text.draw(font.item, rx-text.width(font.item, ">"), yc-text.height(font.item)/2, 255,255,255, is_sel and 200 or 110, ">")
    elseif t == item_type.selected_tick then
        text.draw(font.item, rx-text.width(font.item, "v"), yc-text.height(font.item)/2, c.accentL[1],c.accentL[2],c.accentL[3], is_sel and 255 or 160, "v")
    ...
    elseif t == item_type.color then
        ...
        draw.rect_outline(rx-s, yc-s/2, rx, yc+s/2, 255,255,255, is_sel and 90 or 40, 5)
    elseif t == item_type.search then
        -- magnifier glyph on the far right, query text to its left
        local icx, icy = rx - 8, yc
        draw.circle_outline(icx, icy - 1, 5, 255, 255, 255, is_sel and 180 or 100, 1.5)
        draw.line(icx + 3, icy + 3, icx + 8, icy + 8, 255, 255, 255, is_sel and 180 or 100, 1.5)
        local q = (item.text and item.text ~= "") and item.text or "Search\226\128\166"
        local qa = (item.text and item.text ~= "") and (is_sel and 220 or 160) or (is_sel and 130 or 70)
        local qw = text.width(font.value, q)
        text.draw(font.value, rx - 20 - qw, yc - text.height(font.value)/2, 255, 255, 255, qa, q)
...
-- small chip helper for the action bar / hotkeys
local function chip(x, y, label, font_id)
    local w = text.width(font_id, label)
    local h = text.height(font_id)
    draw.rect(x, y - 2, x + w + 12, y + h + 2, c.surface[1], c.surface[2], c.surface[3], 255, 4)
    text.draw(font_id, x + 6, y, 255, 255, 255, 170, label)
    return w + 12
end
...
local function draw_hotkey_badge(x, y, vk, is_sel, lh)
    ...
    draw.rect(x-4, by-1, x+bw+4, by+bh+2, c.accent[1],c.accent[2],c.accent[3], is_sel and 60 or 36, 4)
    text.draw(font.tiny, x, by, c.accentL[1],c.accentL[2],c.accentL[3], is_sel and 230 or 150, kn)
end

---------------------------------------------------------------------------
-- Search bar / action bar
---------------------------------------------------------------------------
local function draw_search(x, y)
    -- magnifier icon
    local icx, icy = x + l.pad_x + 8, y + l.search_h/2
    draw.circle_outline(icx, icy - 1, 6, 255, 255, 255, 150, 2)
    draw.line(icx + 4, icy + 3, icx + 9, icy + 8, 255, 255, 255, 150, 2)
    -- query: page path
    local page = menu.page_name()
    local parent = menu.page_parent()
    local q = page
    if parent and #parent > 0 and parent ~= page then q = parent .. "  ›  " .. page end
    text.draw(font.title, x + l.pad_x + 26, y + (l.search_h - text.height(font.title))/2, 255, 255, 255, 235, q)
    -- blinking caret
    if math.fmod(ctx.time(), 1.0) < 0.55 then
        local qw = text.width(font.title, q)
        local ch = text.height(font.title)
        draw.rect(x + l.pad_x + 26 + qw + 3, y + (l.search_h - ch)/2 + 2, x + l.pad_x + 26 + qw + 5, y + (l.search_h + ch)/2 - 2, c.accent[1], c.accent[2], c.accent[3], 230)
    end
    -- counter chip on the right
    local counter = string.format("%d of %d", menu.selected_index()+1, menu.item_count())
    local cw = text.width(font.breadcrumb, counter)
    text.draw(font.breadcrumb, x + l.menu_w - l.pad_x - cw, y + (l.search_h - text.height(font.breadcrumb))/2, 255, 255, 255, 110, counter)
    -- divider
    draw.rect(x, y + l.search_h - 1, x + l.menu_w, y + l.search_h, 255, 255, 255, 12)
end

local function draw_actionbar(x, y, item)
    draw.rect(x, y, x + l.menu_w, y + l.foot_h, c.bar[1], c.bar[2], c.bar[3], 255)
    draw.rect(x, y, x + l.menu_w, y + 1, 255, 255, 255, 12)
    -- left: contextual action verb based on selected item
    local verb = str.navigate or "Open"
    if item then
        local t = item.type
        if t == item_type.toggle or t == item_type.float_toggle or t == item_type.int_toggle
            or t == item_type.array_toggle or t == item_type.loop_toggle then verb = str.toggle or "Toggle"
        elseif t == item_type.action then verb = "Run"
        elseif t == item_type.sub_menu then verb = "Open" end
    end
    local cy = y + (l.foot_h - text.height(font.tiny))/2
    local cx = x + l.pad_x
    cx = cx + chip(cx, cy, "Enter", font.tiny) + 6
    text.draw(font.tiny, cx, cy, 255, 255, 255, 150, verb); cx = cx + text.width(font.tiny, verb) + 14
    cx = cx + chip(cx, cy, "Esc", font.tiny) + 6
    text.draw(font.tiny, cx, cy, 255, 255, 255, 150, str.back or "Back")
    -- right brand
    local brand = str.brand or "Nenyoo"
    local bw = text.width(font.tiny, brand)
    text.draw(font.tiny, x + l.menu_w - l.pad_x - bw, cy, c.accentL[1], c.accentL[2], c.accentL[3], 150, brand)
end
...
local function draw_scrollbar(x, list_y, list_h, content_h)
    ...
    draw.rect(sx, thumb_y, sx+sw, thumb_y+thumb_h, 255,255,255, math.floor(120*a), 2)
end
...
local function draw_side_panel(x, y, w, item)
    ...
    local function section(t2)
        ...
        draw.rect(x+14, cy, x+w-14, cy+1, 255,255,255, 12); cy=cy+8
    end
    ...
    draw.rect(x, y, x+w, y+ph, c.bg[1], c.bg[2], c.bg[3], 250, l.round)
...
function draw_menu()
    ...
    local list_h = math.min(l.list_max, content_h+8)
    local total_h = l.search_h + list_h + l.foot_h
    local mx = (sw-l.menu_w)/2
    local my = math.max(40, sh * l.top_frac)

    -- Header drag-to-move: pass natural origin, add returned offset
    local _dox, _doy = menu.drag_header(mx, my, l.menu_w, l.search_h)
    mx = mx + _dox
    my = my + _doy

    frame.menu_x = mx

    -- subtle dim of the whole screen (launcher focus)
    draw.rect(0, 0, sw, sh, 0, 0, 0, 60)
    ...
    -- entrance pop
    local intro = ease_out(clamp((time-open_time)/0.22, 0, 1))

    -- shadow + panel
    draw.rect(mx-8, my-6, mx+l.menu_w+8, my+total_h+14, 0,0,0, math.floor(110*intro), 20)
    draw.rect(mx, my, mx+l.menu_w, my+total_h, c.bg[1], c.bg[2], c.bg[3], 255, l.round)
    draw.rect_outline(mx, my, mx+l.menu_w, my+total_h, 255,255,255, 16, l.round)

    local cy = my
    draw_search(mx, cy); cy = cy + l.search_h
    ...
    -- smooth selection-fill Y
    local sel_y_target = list_y + 4 + sel*l.item_h - scroll
    if sel_y == 0 then sel_y = sel_y_target end
    sel_y = lerp(sel_y, sel_y_target, clamp(dt*20, 0, 1))
    ...
    -- floating selection fill (drawn first, behind all rows)
    if count > 0 and sel_y + l.item_h > list_y and sel_y < list_y + list_h then
        draw.rect(mx+6, sel_y, mx+l.menu_w-6, sel_y+l.item_h-2, c.accent[1], c.accent[2], c.accent[3], 36, 9)
    end
    ...
    for i = 0, count-1 do
        ...
        local t = item.type
        local is_sel = (i == sel)
        local iy = list_y+4 + i*l.item_h - scroll
        ...
        if is_sel then frame.popup_item_y = iy+l.item_h; menu.set_popup_item_y(iy+l.item_h) end
        if hovered and not is_sel then
            draw.rect(mx+6, iy, ibx-6, iy+l.item_h-2, 255,255,255, 10, 9)
        end
        ...
        -- leading dot / icon
        local dotx = mx + l.pad_x + 4
        draw.circle(dotx, iy + l.item_h/2, 3, c.accentL[1], c.accentL[2], c.accentL[3], is_sel and 255 or 80)

        local na = is_sel and 255 or 200
        local tx = mx + l.pad_x + 16
...
function handle_input()
    ...
    if input.key_just_pressed(VK.SPACE) then
        if t==item_type.search then
            if item.text and #item.text > 0 then menu.set_input_buffer(item.text) end
            menu.open_input_popup()
        elseif t==item_type.input_text or t==item_type.input_int or t==item_type.input_float or t==item_type.slider or t==item_type.int_option or t==item_type.float_toggle or t==item_type.int_toggle then menu.open_input_popup() end
    end
...
function draw_array_dropdown_popup()
    ...
    for i = 1, count do
        ...
        if is_sel or hovered then draw.rect(dx+4, iy, dx+dw-4, iy+dh_item, c.accent[1],c.accent[2],c.accent[3], 36, 6) end
...
function draw_color_picker()
    ...
    for i=1,2 do
        ...
        draw.rect(tx, tab_y, tx+tab_w, tab_y+tab_h, c.accent[1], c.accent[2], c.accent[3], active and 40 or 14, 7)
