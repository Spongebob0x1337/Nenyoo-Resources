...
local THEME_NAME="Sunset"
local LAYOUT_MODE="standard"
local TAB_BOTTOM=false
local TABS_ENABLED=true
local HEADER_FILE="Themes\\Sunset\\textures\\header.gif"
local HEADER_PATH="Themes/Sunset/textures/header.gif"
...
local function apply_font_scale(scale)
    ...
    text.set_size(font.title,math.max(1,21*scale))
    text.set_size(font.item,math.max(1,19*scale))
...
local SETTINGS_FILE="stand_import_v3_sunset.ini"
menu.clear_settings()
menu.add_setting_color("Stand Accent",0,0,0,0,"Imported primary colour")
menu.add_setting_color("Stand Background",25,22,22,255,"Imported background colour")
...
menu.add_setting_slider("Stand Position X",1395,0,1920,1,"Main-list X coordinate in Stand HUD pixels")
menu.add_setting_slider("Stand Position Y",520,0,1080,1,"Main-list Y coordinate in Stand HUD pixels")
...
local function load_settings()
    ...
    for line in body:gmatch("[^\r\n]+") do
        ...
        elseif key=="position_x" then menu.set_setting("Stand Position X",tonumber(value) or 1395)
        elseif key=="position_y" then menu.set_setting("Stand Position Y",tonumber(value) or 520)
        end
    end
end
...
local function serialize_settings()
    ...
    local background=setting("Stand Background") or {r=25,g=22,b=22,a=255}
    ...
    local position_x=setting("Stand Position X") or {f_val=1395}
    local position_y=setting("Stand Position Y") or {f_val=520}
    return table.concat({
...
local function reset_settings()
    ...
    menu.set_setting("Stand Background",25,22,22,255)
    menu.set_setting("Stand Width",380)
    menu.set_setting("Stand Rows",12)
    menu.set_setting("Stand Row Height",34)
    menu.set_setting("Stand Position X",1395)
    menu.set_setting("Stand Position Y",520)
...
function draw_menu()
    ...
    local position_x=clamp(position_x_setting and position_x_setting.f_val or 1395,0,1920)*hud_sx
    local position_y=clamp(position_y_setting and position_y_setting.f_val or 520,0,1080)*hud_sy
    ...
    local page_h=SUBHEADER_PATH~="" and native_height(subheader_handle,width,36*hud_sy)
        or (TABS_ENABLED and 36*hud_sy or 0)
    ...
    if 230>0 and 2>0 then
        draw.rect_outline(x,content_y,x+width,content_y+body_h,124,37,186,230,0,2)
    end
    ...
    for row=0,visible-1 do
        ...
        if item then
            ...
            if 230>0 then
                draw.rect(x,yy+row_h-1,x+width,yy+row_h,124,37,186,math.min(230,80))
            end
