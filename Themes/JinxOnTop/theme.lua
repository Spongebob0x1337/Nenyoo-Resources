...
local THEME_NAME="JinxOnTop"
local LAYOUT_MODE="standard"
local TAB_BOTTOM=false
local TABS_ENABLED=true
local HEADER_FILE="Themes\\JinxOnTop\\textures\\header.gif"
local HEADER_PATH="Themes/JinxOnTop/textures/header.gif"
...
local SETTINGS_FILE="stand_import_v3_jinxontop.ini"
menu.clear_settings()
menu.add_setting_color("Stand Accent",233,237,165,255,"Imported primary colour")
...
menu.add_setting_slider("Stand Position X",50,0,1920,1,"Main-list X coordinate in Stand HUD pixels")
menu.add_setting_slider("Stand Position Y",209,0,1080,1,"Main-list Y coordinate in Stand HUD pixels")
...
local function load_settings()
    ...
    for line in body:gmatch("[^\r\n]+") do
        ...
        elseif key=="position_x" then menu.set_setting("Stand Position X",tonumber(value) or 50)
        elseif key=="position_y" then menu.set_setting("Stand Position Y",tonumber(value) or 209)
        end
    end
end

local function serialize_settings()
    local accent=setting("Stand Accent") or {r=233,g=237,b=165,a=255}
    ...
    local position_x=setting("Stand Position X") or {f_val=50}
    local position_y=setting("Stand Position Y") or {f_val=209}
    return table.concat({
...
local function reset_settings()
    menu.set_setting("Stand Accent",233,237,165,255)
    ...
    menu.set_setting("Stand Position X",50)
    menu.set_setting("Stand Position Y",209)
...
function draw_menu()
    ...
    local position_x=clamp(position_x_setting and position_x_setting.f_val or 50,0,1920)*hud_sx
    local position_y=clamp(position_y_setting and position_y_setting.f_val or 209,0,1080)*hud_sy
    ...
    if 255>0 and 2>0 then
        draw.rect_outline(x,content_y,x+width,content_y+body_h,0,0,0,255,0,2)
    end
    ...
    if page_h>0 then
        ...
        if TABS_ENABLED then
            ...
            text.draw(font.small,x+width-10-text.width(font.small,counter),
                page_y+(page_h-text.height(font.small))*0.5,190,237,200,255,counter)
        end
    end
        ...
        if item then
            ...
            if item.is_header then
                ...
                text.draw_ellipsis(font.item,text_x,yy+(row_h-text.height(font.item))*0.5,
                    active and 0 or 190,active and 0 or 237,active and 0 or 200,active and 255 or 255,
                ...
                if toggle_type then
                    ...
                    if toggle_handle and toggle_handle>0 then
                        ...
                        draw.rect_outline(indicator_x-5,yy+row_h*0.5-5,indicator_x+5,yy+row_h*0.5+5,
                            active and 0 or 190,active and 0 or 237,active and 0 or 200,210,1,1)
                ...
                elseif action_mark then
                    ...
                    if action_handle and action_handle>0 then
                        ...
                        text.draw(font.small,x+width-10-text.width(font.small,mark),yy+(row_h-text.height(font.small))*0.5,
                            active and 0 or 190,active and 0 or 237,active and 0 or 200,255,mark)
                    end
                end
                if value~="" then
                    ...
                    text.draw(font.value,right-text.width(font.value,value),yy+(row_h-text.height(font.value))*0.5,
                        active and 0 or 190,active and 0 or 237,active and 0 or 200,255,value)
                end
            end
    ...
    if footer_handle and footer_handle>0 then
        ...
        text.draw(font.desc,x+9,desc_y+7,190,237,200,255,description,width-18)
        draw.pop_clip()
    end
