...
-- SIGNAL-77 renderer
--
-- The legacy terminal code above is retained for its complete, proven modal
-- editors. Everything visible in the normal menu is replaced below.
-------------------------------------------------------------------------------

local SIGNAL_FILE = "signal77_theme.ini"
local SIGNAL_W, SIGNAL_H = 760, 548
local SIGNAL_RAIL = 184
local SIGNAL_HEADER = 52
local SIGNAL_TELEMETRY = 48
local SIGNAL_CRUMB = 30
local SIGNAL_DESC = 48
local SIGNAL_FOOTER = 30

local SIGNAL_DEFS = {
    {name="Animation Speed", kind="num", default=1.00, min=0.25, max=2.50, step=0.05,
        desc="Speed of waveforms, scan bars and signal motion"},
    {name="CRT Intensity", kind="num", default=0.80, min=0.00, max=1.00, step=0.05,
        desc="Strength of scanlines, noise and phosphor effects"},
    {name="Menu Scale", kind="num", default=1.00, min=0.75, max=1.25, step=0.05,
        desc="Scale the complete Signal-77 console"},
    {name="Scanlines", kind="tog", default=true, desc="Draw animated CRT scanlines"},
    {name="Navigation Glitches", kind="tog", default=true, desc="Brief signal breakup after navigation"},
    {name="Phosphor Bloom", kind="tog", default=true, desc="Draw a soft amber glow around the console"},
    {name="Boot Sequence", kind="tog", default=true, desc="Show the three-second startup sequence"},
    {name="Reduced Motion", kind="tog", default=false, desc="Disable flicker, glitches and rapid sweeps"},
}

local signal_last_sig = ""
local signal_save_pending = false
local signal_save_at = 0
local signal_home = nil
local signal_rail_index = 0
local signal_focus = 0 -- 0 rail, 1 content
local signal_active_module = ""
local signal_last_page = ""
local signal_last_selected = -1
local signal_glitch_at = -10
local signal_boot_started = -1
local signal_boot_skipped = false
local signal_scroll = 0
local signal_scroll_target = 0
local signal_drag_slider = -1
local signal_drag_x, signal_drag_w = 0, 1
local signal_last_scale = -1

local function signal_setting(name)
    return menu.get_setting(name)
end

local function signal_num(name, fallback)
    local s = signal_setting(name)
    return s and s.f_val or fallback
end

local function signal_toggle(name, fallback)
    local s = signal_setting(name)
    if s and s.on ~= nil then return s.on end
    return fallback
end

local function signal_register_settings()
    menu.clear_settings()
    menu.add_setting_submenu("Signal Motion", "CRT animation and display behavior")
    for _,d in ipairs(SIGNAL_DEFS) do
        if d.kind == "num" then
            menu.add_sub_slider(d.name, d.default, d.min, d.max, d.step, d.desc)
        else
            menu.add_sub_toggle(d.name, d.default, d.desc)
        end
    end
    menu.add_setting_action("Reset Theme", "Restore all Signal-77 settings")
end

local function signal_serialize()
    local out = {"version=1"}
    for _,d in ipairs(SIGNAL_DEFS) do
        if d.kind == "num" then
            out[#out+1] = d.name .. "=" .. tostring(signal_num(d.name, d.default))
        else
            out[#out+1] = d.name .. "=" .. (signal_toggle(d.name, d.default) and "1" or "0")
        end
    end
    return table.concat(out, "\n")
end

local function signal_load_settings()
    local data = file.read(SIGNAL_FILE)
    if not data or not data:match("^version=1") then return end
    local known = {}
    for _,d in ipairs(SIGNAL_DEFS) do known[d.name] = d end
    for line in data:gmatch("[^\r\n]+") do
        local key,value = line:match("^(.-)=(.*)$")
        local d = key and known[key]
        if d and d.kind == "num" then
            local number = tonumber(value)
            if number then menu.set_setting(key, number) end
        elseif d and d.kind == "tog" then
            menu.set_setting(key, value == "1")
        end
    end
end

local function signal_reset_settings()
    for _,d in ipairs(SIGNAL_DEFS) do menu.set_setting(d.name, d.default) end
    file.remove(SIGNAL_FILE)
    signal_last_sig = signal_serialize()
    signal_save_pending = false
    signal_boot_started = ctx.time()
    signal_boot_skipped = false
end

local function signal_apply_fonts(scale)
    if math.abs(scale-signal_last_scale) < 0.001 then return end
    text.set_font_family("Consolas")
    text.set_size(font.title, math.floor(20*scale+0.5)); text.set_weight(font.title, 700)
    text.set_size(font.item, math.floor(14*scale+0.5)); text.set_weight(font.item, 500)
    text.set_size(font.value, math.floor(12*scale+0.5)); text.set_weight(font.value, 600)
    text.set_size(font.breadcrumb, math.floor(12*scale+0.5)); text.set_weight(font.breadcrumb, 700)
    text.set_size(font.desc, math.floor(11*scale+0.5)); text.set_weight(font.desc, 400)
    text.set_size(font.label, math.floor(11*scale+0.5)); text.set_weight(font.label, 600)
    text.set_size(font.small, math.floor(10*scale+0.5)); text.set_weight(font.small, 500)
    text.set_size(font.tiny, math.floor(9*scale+0.5)); text.set_weight(font.tiny, 500)
    text.set_size(font.tagline, math.floor(11*scale+0.5)); text.set_weight(font.tagline, 600)
    signal_last_scale = scale
end

local function signal_hit(x1,y1,x2,y2)
    local mx,my = input.mouse_x(),input.mouse_y()
    return mx>=x1 and mx<=x2 and my>=y1 and my<=y2
end

local function signal_color(alpha_value)
    return 255,182,77,alpha_value
end

local function signal_draw_grid(x,y,w,h,intensity)
    local a = math.floor(22*intensity)
    for gx=x,w+x,24 do draw.line(gx,y,gx,y+h,103,216,255,a,1) end
    for gy=y,y+h,24 do draw.line(x,gy,x+w,gy,103,216,255,a,1) end
end

local function signal_draw_wave(x,y,w,h,time_value,intensity)
    local steps = 64
    local last_x,last_y = x,y+h*0.5
    for i=1,steps do
        local p = i/steps
        local px = x+p*w
        local py = y+h*0.5 + math.sin(p*math.pi*8+time_value*2.7)*h*0.22
            + math.sin(p*math.pi*21-time_value*1.4)*h*0.06
        draw.line(last_x,last_y,px,py,103,216,255,math.floor(190*intensity),1.2)
        last_x,last_y = px,py
    end
end

local function signal_draw_noise(x,y,w,h,time_value,intensity,reduced)
    if reduced or intensity <= 0 then return end
    local seed = math.floor(time_value*18)
    local count = math.floor(18+intensity*34)
    for i=1,count do
        local px = x + ((i*97 + seed*43) % math.max(1,math.floor(w)))
        local py = y + ((i*53 + seed*29) % math.max(1,math.floor(h)))
        local len = 2 + ((i*11+seed) % 17)
        local ice = i%3==0
        draw.line(px,py,math.min(x+w,px+len),py,
            ice and 103 or 255, ice and 216 or 182, ice and 255 or 77,
            math.floor((15+(i%5)*7)*intensity),1)
    end
end

local function signal_draw_scanlines(x,y,w,h,time_value,intensity,reduced)
    if not signal_toggle("Scanlines",true) or intensity <= 0 then return end
    local alpha_value = math.floor((reduced and 13 or 23)*intensity)
    for sy=y,y+h,4 do draw.rect(x,sy,x+w,sy+1,0,0,0,alpha_value) end
    if not reduced then
        local scan = y-50 + ((time_value*38) % (h+100))
        draw.rect_gradient(x,scan-20,x+w,scan,
            255,182,77,0,103,216,255,0,103,216,255,20,255,182,77,20)
        draw.rect_gradient(x,scan,x+w,scan+20,
            255,182,77,20,103,216,255,20,103,216,255,0,255,182,77,0)
    end
end

local function signal_draw_bloom(x,y,w,h,intensity)
    if not signal_toggle("Phosphor Bloom",true) then return end
    for i=8,1,-1 do
        draw.rect_outline(x-i,y-i,x+w+i,y+h+i,255,182,77,
            math.floor((9-i)*3*intensity),3+i,1)
    end
end

local function signal_home_handles()
    if not signal_home or #signal_home==0 then signal_home=items.page_items("Home") or {} end
    return signal_home
end

local function signal_activate_current()
    local item = menu.get_item(menu.selected_index())
    if item and item.type==item_type.action and item.name=="Reset Theme" then
        signal_reset_settings()
        notify.push("Signal-77","Theme settings restored",1)
        return
    end
    menu.activate()
end

local function signal_open_module(index)
    local handles = signal_home_handles()
    if #handles==0 then return end
    signal_rail_index = math.max(0,math.min(index,#handles-1))
    local handle = handles[signal_rail_index+1]
    signal_active_module = items.name(handle) or ""
    signal_focus = 1
    signal_glitch_at = ctx.time()
    items.activate(handle)
end

local function signal_draw_boot(x,y,w,h,now,reduced)
    if signal_boot_started < 0 then signal_boot_started=now end
    if signal_boot_skipped or reduced or not signal_toggle("Boot Sequence",true) then return false end
    local elapsed = now-signal_boot_started
    if elapsed>=3 then return false end
    draw.rect(x,y,x+w,y+h,1,3,3,252,4)
    local cx=x+30
    local cy=y+30
    local lines={
        "SIGNAL BIOS 7.7.04",
        "CHECKING PHOSPHOR MATRIX ........ OK",
        "SYNCING TELEMETRY BUS .......... OK",
        "MOUNTING NENYOO MODULES ........ OK",
        "CALIBRATING INPUT CHANNELS ..... OK",
    }
    local visible=math.min(#lines,math.floor(elapsed*3.2))
    for i=1,visible do
        text.draw(font.small,cx,cy+(i-1)*18,255,182,77,220,lines[i])
    end
    if elapsed>1.65 then
        local title="NENYOO // SIGNAL-77"
        local reveal=math.floor(#title*math.min(1,(elapsed-1.65)/0.75))
        text.draw(font.title,cx,y+h*0.54,255,182,77,255,title:sub(1,reveal))
        signal_draw_wave(cx,y+h*0.67,w-60,52,now,math.min(1,(elapsed-1.65)*1.5))
    end
    local progress=math.min(1,elapsed/3)
    draw.rect(cx,y+h-48,x+w-30,y+h-42,39,31,18,255)
    draw.rect(cx,y+h-48,cx+(w-60)*progress,y+h-42,103,216,255,230)
    text.draw(font.tiny,cx,y+h-32,103,216,255,180,"ENTER / CLICK TO BYPASS STARTUP")
    signal_draw_scanlines(x,y,w,h,now,0.8,false)
    return true
end

local function signal_draw_rail(x,y,w,h,now,scale)
    draw.rect(x,y,x+w,y+h,3,7,7,244)
    draw.line(x+w-1,y,x+w-1,y+h,255,182,77,110,1)
    text.draw(font.tiny,x+14,y+12,103,216,255,185,"MODULE BUS // 0x77")
    local handles=signal_home_handles()
    local row_h=31*scale
    local row_y=y+34*scale
    local current=menu.page_name() or ""
    local parent=menu.page_parent() or ""
    for index,handle in ipairs(handles) do
        local zero=index-1
        local name=items.name(handle) or "UNKNOWN"
        if current==name or parent==name then
            signal_active_module=name
            signal_rail_index=zero
        end
        local active=(name==signal_active_module) or (current==name) or (parent==name)
        local focused=(signal_focus==0 and zero==signal_rail_index)
        local hovered=signal_hit(x,row_y,x+w,row_y+row_h)
        if active then draw.rect(x,row_y,x+w,row_y+row_h,255,182,77,24) end
        if focused or hovered then
            draw.rect(x,row_y,x+w,row_y+row_h,103,216,255,focused and 24 or 13)
            draw.rect(x,row_y,x+3,row_y+row_h,focused and 103 or 255,focused and 216 or 182,focused and 255 or 77,230)
        elseif active then
            draw.rect(x,row_y,x+3,row_y+row_h,255,182,77,220)
        end
        local prefix=active and "[ON] " or "[--] "
        local tc=active and {255,182,77} or {117,105,75}
        if focused then tc={103,216,255} end
        text.draw(font.item,x+12,row_y+(row_h-text.height(font.item))*0.5,
            tc[1],tc[2],tc[3],focused and 255 or 220,prefix..string.upper(name))
        if hovered and input.mouse_clicked(0) and menu.popup_mode()==0 then signal_open_module(zero) end
        row_y=row_y+row_h
        if row_y+row_h>y+h-44 then break end
    end
    local pulse=signal_toggle("Reduced Motion",false) and 1 or (math.sin(now*3)*0.5+0.5)
    draw.circle(x+17,y+h-20,3,103,216,255,math.floor(120+120*pulse))
    text.draw(font.tiny,x+28,y+h-26,103,216,255,150,signal_focus==0 and "RAIL FOCUS" or "CONTENT LINKED")
end

local function signal_draw_overview(x,y,w,h,now,intensity)
    signal_draw_grid(x,y,w,h,intensity)
    text.draw(font.breadcrumb,x+18,y+15,255,182,77,230,"SYSTEM OVERVIEW")
    text.draw(font.tiny,x+18,y+38,103,216,255,170,"SELECT A MODULE FROM THE LEFT BUS")
    signal_draw_wave(x+18,y+72,w-36,72,now,intensity)
    local box_y=y+170
    local box_w=(w-50)/3
    local labels={{"LINK","STABLE"},{"FRAME",string.format("%.1f",ctx.fps())},{"EDITION",string.upper(ctx.edition and ctx.edition() or "LEGACY")}}
    for i,data in ipairs(labels) do
        local bx=x+18+(i-1)*(box_w+7)
        draw.rect(bx,box_y,bx+box_w,box_y+62,5,12,12,230,2)
        draw.rect_outline(bx,box_y,bx+box_w,box_y+62,103,216,255,70,2,1)
        text.draw(font.tiny,bx+10,box_y+9,103,216,255,130,data[1])
        text.draw(font.item,bx+10,box_y+30,255,182,77,230,data[2])
    end
    local log_y=box_y+88
    text.draw(font.tiny,x+18,log_y,103,216,255,135,"LIVE SIGNAL")
    for i=1,5 do
        local v=math.floor((math.sin(now*(0.7+i*0.11)+i)*0.5+0.5)*18)+4
        draw.rect(x+18,log_y+18+i*15,x+18+v*8,log_y+23+i*15,255,182,77,90+i*20)
    end
end

local function signal_draw_item_rows(x,y,w,h,now,dt,scale)
    local count=menu.item_count()
    local row_h=34*scale
    local content_h=count*row_h
    local selected=menu.selected_index()
    if selected~=signal_last_selected then
        local top=selected*row_h
        if top<signal_scroll_target then signal_scroll_target=top end
        if top+row_h>signal_scroll_target+h then signal_scroll_target=top+row_h-h end
        signal_last_selected=selected
        signal_glitch_at=now
    end
    local max_scroll=math.max(0,content_h-h)
    signal_scroll_target=clamp(signal_scroll_target,0,max_scroll)
    signal_scroll=lerp(signal_scroll,signal_scroll_target,clamp(dt*15,0,1))
    if signal_hit(x,y,x+w,y+h) and menu.popup_mode()==0 then
        local wheel=input.mouse_wheel()
        if wheel~=0 then signal_scroll_target=clamp(signal_scroll_target-wheel*row_h*3,0,max_scroll) end
    end
    local mx,my=input.mouse_x(),input.mouse_y()
    if signal_drag_slider>=0 then
        local item=menu.get_item(signal_drag_slider)
        if item and input.mouse_down(0) then
            local pct=clamp((mx-signal_drag_x)/signal_drag_w,0,1)
            local raw=item.f_min+pct*(item.f_max-item.f_min)
            if item.f_step and item.f_step>0 then raw=item.f_min+math.floor((raw-item.f_min)/item.f_step+0.5)*item.f_step end
            menu.set_f_val(signal_drag_slider,clamp(raw,item.f_min,item.f_max))
        else signal_drag_slider=-1 end
    end
    draw.push_clip(x,y,x+w,y+h)
    for i=0,count-1 do
        local iy=y+i*row_h-signal_scroll
        if iy+row_h>=y and iy<=y+h then
            local item=menu.get_item(i)
            if item then
                local active=(i==selected)
                local hovered=signal_hit(x,iy,x+w,iy+row_h)
                if item.is_header then
                    draw.line(x+10,iy+row_h-3,x+w-10,iy+row_h-3,255,182,77,60,1)
                    text.draw(font.tiny,x+13,iy+(row_h-text.height(font.tiny))*0.5,103,216,255,180,string.upper(item.name or "SECTION"))
                else
                    if active then
                        draw.rect_gradient(x+2,iy,x+w-2,iy+row_h,
                            255,182,77,42,255,182,77,8,255,182,77,8,255,182,77,42)
                        draw.rect(x+2,iy,x+5,iy+row_h,255,182,77,255)
                        frame.popup_item_y=iy+row_h
                        menu.set_popup_item_y(iy+row_h)
                    elseif hovered then
                        draw.rect(x+2,iy,x+w-2,iy+row_h,103,216,255,12)
                    end
                    local marker=active and ">" or "·"
                    local tc=active and {255,213,142} or {174,153,111}
                    text.draw(font.item,x+13,iy+(row_h-text.height(font.item))*0.5,tc[1],tc[2],tc[3],active and 255 or 215,marker.." "..(item.name or ""))
                    draw_widget(x+w-14,iy+row_h*0.5,item,active)
                    if item.hotkey and item.hotkey~=0 then
                        draw_hotkey_badge(x+18+text.width(font.item,marker.." "..(item.name or "")),iy+(row_h-text.height(font.item))*0.5,item.hotkey,active,text.height(font.item))
                    end
                    if hovered and input.mouse_clicked(0) and menu.popup_mode()==0 and signal_drag_slider<0 then
                        menu.set_selected(i)
                        signal_focus=1
                        if item.type==item_type.slider then
                            local value_w=text.width(font.value,string.format("%.1f",item.f_val))
                            local right=x+w-14-value_w-10
                            local left=right-BAR_W
                            if mx>=left and mx<=right then
                                signal_drag_slider=i; signal_drag_x=left; signal_drag_w=BAR_W
                            elseif active then signal_activate_current() end
                        elseif active then signal_activate_current() end
                    end
                end
            end
    ...
    if max_scroll>0 then
        local thumb=math.max(18,h*(h/content_h))
        local sy=y+(h-thumb)*(signal_scroll/max_scroll)
        draw.rect(x+w-3,y,x+w,y+h,37,31,20,150)
        draw.rect(x+w-3,sy,x+w,sy+thumb,103,216,255,190)
    end
end

local function signal_draw_glitch(x,y,w,h,now,intensity,reduced)
    if reduced or not signal_toggle("Navigation Glitches",true) then return end
    local elapsed=now-signal_glitch_at
    if elapsed<0 or elapsed>0.18 then return end
    local strength=(1-elapsed/0.18)*intensity
    for i=1,5 do
        local gy=y+((i*83+math.floor(now*900))%math.max(1,math.floor(h)))
        local gh=2+(i%3)*2
        local shift=((i%2==0) and 1 or -1)*(3+i*2)*strength
        draw.rect(x+shift,gy,x+w+shift,gy+gh,i%2==0 and 103 or 255,i%2==0 and 216 or 182,i%2==0 and 255 or 77,math.floor(42*strength))
    end
end

function draw_menu()
    if not menu.is_visible() then return end
    local now,dt=ctx.time(),ctx.delta()
    local scale=signal_num("Menu Scale",1.0)
    local speed=signal_num("Animation Speed",1.0)
    local intensity=signal_num("CRT Intensity",0.8)
    local reduced=signal_toggle("Reduced Motion",false)
    signal_apply_fonts(scale)
    c={accent={255,182,77,255},accentD={116,83,43,255},bg={4,8,7,255},amber={255,182,77,255},red={230,74,58,255},ice={103,216,255,255}}
    l={menu_w=SIGNAL_W*scale,item_h=34*scale,header_h=SIGNAL_HEADER*scale,footer_h=SIGNAL_FOOTER*scale,list_max=330*scale,pad_x=14*scale,scan=true,glow=true}
    theme.set_body_bg(2,5,5,245)
    theme.set_menu_bg(4,8,7,245)
    theme.set_accent_palette(255,182,77,255)

    local sw,sh=ctx.screen_w(),ctx.screen_h()
    local w,h=SIGNAL_W*scale,SIGNAL_H*scale
    local x,y=(sw-w)*0.5,math.max(18,(sh-h)*0.5)
    local dx,dy=menu.drag_header(x,y,w,SIGNAL_HEADER*scale)
    x=x+dx; y=y+dy
    frame.menu_x=x+SIGNAL_RAIL*scale
    l.menu_w=w-SIGNAL_RAIL*scale

    local page=menu.page_name() or "Home"
    if page~=signal_last_page then
        signal_last_page=page
        signal_scroll=0; signal_scroll_target=0
        signal_glitch_at=now
        if page=="Home" then signal_focus=0 else signal_focus=1 end
    end

    local motion_time=reduced and 0 or now*speed
    signal_draw_bloom(x,y,w,h,intensity)
    draw.rect(x,y,x+w,y+h,4,8,7,248,4)
    draw.rect_outline(x,y,x+w,y+h,255,182,77,150,4,1)
    signal_draw_grid(x,y,w,h,intensity)

    local header_h=SIGNAL_HEADER*scale
    draw.rect(x,y,x+w,y+header_h,5,11,11,250,4)
    draw.line(x,y+header_h-1,x+w,y+header_h-1,103,216,255,125,1)
    text.draw(font.title,x+18*scale,y+(header_h-text.height(font.title))*0.5,255,182,77,255,"NENYOO // SIGNAL-77")
    local status="LINK:STABLE   BUS:0x77   "..string.upper(ctx.edition and ctx.edition() or "LEGACY")
    text.draw(font.tiny,x+w-18*scale-text.width(font.tiny,status),y+(header_h-text.height(font.tiny))*0.5,103,216,255,185,status)

    local rail_y=y+header_h
    local rail_h=h-header_h-SIGNAL_FOOTER*scale
    signal_draw_rail(x,rail_y,SIGNAL_RAIL*scale,rail_h,motion_time,scale)

    local right_x=x+SIGNAL_RAIL*scale
    local right_w=w-SIGNAL_RAIL*scale
    local telemetry_h=SIGNAL_TELEMETRY*scale
    draw.rect(right_x,rail_y,right_x+right_w,rail_y+telemetry_h,3,9,10,238)
    signal_draw_wave(right_x+12*scale,rail_y+6*scale,right_w-24*scale,telemetry_h-12*scale,motion_time,intensity)
    text.draw(font.tiny,right_x+12*scale,rail_y+7*scale,255,182,77,170,"OSC // CH-A")
    text.draw(font.tiny,right_x+right_w-76*scale,rail_y+7*scale,103,216,255,150,string.format("%03d FPS",math.floor(ctx.fps()+0.5)))

    local crumb_y=rail_y+telemetry_h
    local crumb_h=SIGNAL_CRUMB*scale
    draw.rect(right_x,crumb_y,right_x+right_w,crumb_y+crumb_h,7,12,11,245)
    draw.line(right_x,crumb_y+crumb_h-1,right_x+right_w,crumb_y+crumb_h-1,255,182,77,85,1)
    local parent=menu.page_parent() or "ROOT"
    text.draw(font.breadcrumb,right_x+14*scale,crumb_y+(crumb_h-text.height(font.breadcrumb))*0.5,255,182,77,225,string.upper(parent.." / "..page))
    text.draw(font.tiny,right_x+right_w-145*scale,crumb_y+(crumb_h-text.height(font.tiny))*0.5,103,216,255,150,signal_focus==0 and "[TAB] CONTENT" or "[TAB] MODULES")

    local footer_h=SIGNAL_FOOTER*scale
    local desc_h=SIGNAL_DESC*scale
    local body_y=crumb_y+crumb_h
    local desc_y=y+h-footer_h-desc_h
    local body_h=desc_y-body_y
    menu.set_content_rect(right_x,body_y,right_w,body_h)
    if page=="Home" then
        signal_draw_overview(right_x,body_y,right_w,body_h,motion_time,intensity)
    else
        signal_draw_item_rows(right_x,body_y,right_w,body_h,now,dt,scale)
    end

    draw.rect(right_x,desc_y,right_x+right_w,desc_y+desc_h,3,8,8,248)
    draw.line(right_x,desc_y,right_x+right_w,desc_y,103,216,255,75,1)
    local selected=menu.get_item(menu.selected_index())
    local description=(selected and selected.desc and selected.desc~="") and selected.desc or "Awaiting operator input"
    draw.circle(right_x+16*scale,desc_y+desc_h*0.5,3*scale,255,182,77,220)
    text.draw_ellipsis(font.desc,right_x+28*scale,desc_y+(desc_h-text.height(font.desc))*0.5,166,181,164,210,description,right_w-44*scale)

    local footer_y=y+h-footer_h
    draw.rect(x,footer_y,x+w,y+h,2,6,6,252,4)
    draw.line(x,footer_y,x+w,footer_y,255,182,77,100,1)
    text.draw(font.tiny,x+14*scale,footer_y+(footer_h-text.height(font.tiny))*0.5,255,182,77,150,"TAB FOCUS  |  ENTER EXECUTE  |  ESC RETURN")
    local count=menu.item_count()
    local counter=page=="Home" and ("MODULES "..#signal_home_handles()) or string.format("%02d/%02d",math.min(menu.selected_index()+1,count),count)
    text.draw(font.tiny,x+w-14*scale-text.width(font.tiny,counter),footer_y+(footer_h-text.height(font.tiny))*0.5,103,216,255,180,counter)

    signal_draw_noise(x,y,w,h,motion_time,intensity,reduced)
    signal_draw_scanlines(x,y,w,h,motion_time,intensity,reduced)
    signal_draw_glitch(x,y,w,h,now,intensity,reduced)

    local booting=signal_draw_boot(x,y,w,h,now,reduced)
    if not booting then
        local pm=menu.popup_mode()
        if pm==1 then draw_input_popup() end
        if pm==2 then draw_color_picker() end
    ...
    local signature=signal_serialize()
    if signature~=signal_last_sig then signal_last_sig=signature; signal_save_pending=true; signal_save_at=now end
    if signal_save_pending and now-signal_save_at>0.4 then file.write(SIGNAL_FILE,signal_last_sig); signal_save_pending=false end
end
...
function handle_input()
    ...
    local now=ctx.time()
    local reduced=signal_toggle("Reduced Motion",false)
    local booting=not signal_boot_skipped and not reduced and signal_toggle("Boot Sequence",true)
        and signal_boot_started>=0 and now-signal_boot_started<3
    if booting then
        if input.key_just_pressed(VK.RETURN) or input.key_just_pressed(VK.SPACE)
            or input.key_just_pressed(VK.ESCAPE) or input.mouse_clicked(0) then signal_boot_skipped=true end
        return
    end
    local pm=menu.popup_mode()
    if pm==1 then handle_input_popup(); return end
    ...
    if input.key_just_pressed(VK.TAB) then signal_focus=1-signal_focus; signal_glitch_at=now; return end
    local page=menu.page_name() or "Home"
    if signal_focus==0 then
        local handles=signal_home_handles()
        if input.key_pressed(VK.UP) and #handles>0 then signal_rail_index=(signal_rail_index-1)%#handles end
        if input.key_pressed(VK.DOWN) and #handles>0 then signal_rail_index=(signal_rail_index+1)%#handles end
        if input.key_just_pressed(VK.RETURN) then signal_open_module(signal_rail_index) end
        if (input.key_just_pressed(VK.BACK) or input.key_just_pressed(VK.ESCAPE)) and page~="Home" then menu.go_back() end
        return
    end

    if input.key_pressed(VK.UP) then menu.move_selection(-1) end
    if input.key_pressed(VK.DOWN) then menu.move_selection(1) end
    if input.key_just_pressed(VK.RETURN) then signal_activate_current() end
    if input.key_just_pressed(VK.BACK) or input.key_just_pressed(VK.ESCAPE) then menu.go_back(); signal_glitch_at=now end
    local selected=menu.selected_index()
    local item=menu.get_item(selected)
    if not item then return end
    local t=item.type
    if input.key_just_pressed(VK.SPACE) then
        if t==item_type.input_text or t==item_type.search or t==item_type.input_int or t==item_type.input_float
            or t==item_type.slider or t==item_type.int_option or t==item_type.float_toggle or t==item_type.int_toggle then menu.open_input_popup() end
    end
    if input.key_just_pressed(0x48) and t~=item_type.sub_menu then menu.set_popup_mode(4) end
    if input.key_just_pressed(VK.DELETE) and item.hotkey and item.hotkey~=0 then
        menu.set_hotkey(selected,0); notify.push(item.name,str.cleared or "cleared",0); menu.save_hotkeys(); menu.rebuild_features()
    end
    if input.key_pressed(VK.LEFT) or input.key_pressed(VK.RIGHT) then
        local direction=input.key_pressed(VK.RIGHT) and 1 or -1
        if t==item_type.slider or t==item_type.float_toggle then
            local step=item.f_step~=0 and item.f_step or 0.1
            menu.set_f_val(selected,clamp(item.f_val+step*direction,item.f_min,item.f_max))
        elseif t==item_type.int_option or t==item_type.int_toggle then
            local step=item.i_step~=0 and item.i_step or 1
            menu.set_i_val(selected,clamp(item.i_val+step*direction,item.i_min,item.i_max))
        elseif t==item_type.loop_option or t==item_type.loop_toggle then
            local index=item.value_index+direction
            if index>=item.value_count then index=0 elseif index<0 then index=item.value_count-1 end
            menu.set_value_index(selected,index)
        elseif t==item_type.array_option or t==item_type.array_toggle then menu.open_array_popup() end
    end
end

signal_register_settings()
signal_load_settings()
signal_last_sig=signal_serialize()
