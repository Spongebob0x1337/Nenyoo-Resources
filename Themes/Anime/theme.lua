...
local THEME_NAME="Anime"
local LAYOUT_MODE="standard"
local TAB_BOTTOM=false
local TABS_ENABLED=true
local HEADER_FILE="Themes\\Anime\\textures\\header.gif"
local HEADER_PATH="Themes/Anime/textures/header.gif"
...
local SETTINGS_FILE="stand_import_v3_anime.ini"
menu.clear_settings()
menu.add_setting_color("Stand Accent",59,0,255,255,"Imported primary colour")
menu.add_setting_color("Stand Background",60,62,63,245,"Imported background colour")
...
menu.add_setting_slider("Stand Position X",286,0,1920,1,"Main-list X coordinate in Stand HUD pixels")
menu.add_setting_slider("Stand Position Y",260,0,1080,1,"Main-list Y coordinate in Stand HUD pixels")
...
local function load_settings()
    ...
    for line in body:gmatch("[^\r\n]+") do
        ...
        elseif key=="position_x" then menu.set_setting("Stand Position X",tonumber(value) or 286)
        elseif key=="position_y" then menu.set_setting("Stand Position Y",tonumber(value) or 260)
        end
    end
end

local function serialize_settings()
    local accent=setting("Stand Accent") or {r=59,g=0,b=255,a=255}
    local background=setting("Stand Background") or {r=60,g=62,b=63,a=245}
    ...
    local position_x=setting("Stand Position X") or {f_val=286}
    local position_y=setting("Stand Position Y") or {f_val=260}
    return table.concat({
...
local function reset_settings()
    menu.set_setting("Stand Accent",59,0,255,255)
    menu.set_setting("Stand Background",60,62,63,245)
    menu.set_setting("Stand Width",392)
    menu.set_setting("Stand Rows",16)
    menu.set_setting("Stand Row Height",31)
    menu.set_setting("Stand Position X",286)
    menu.set_setting("Stand Position Y",260)
...
function draw_menu()
    ...
    local position_x=clamp(position_x_setting and position_x_setting.f_val or 286,0,1920)*hud_sx
    local position_y=clamp(position_y_setting and position_y_setting.f_val or 260,0,1080)*hud_sy
    ...
    if 255>0 and 3>0 then
        draw.rect_outline(x,content_y,x+width,content_y+body_h,59,0,255,255,5,3)
    end
    ...
    for row=0,visible-1 do
        ...
        if item then
            ...
            if 255>0 then
                draw.rect(x,yy+row_h-1,x+width,yy+row_h,59,0,255,math.min(255,80))
            end
