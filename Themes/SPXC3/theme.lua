...
local THEME_NAME="SPXC3"
local LAYOUT_MODE="standard"
local TAB_BOTTOM=false
local TABS_ENABLED=true
local HEADER_FILE="Themes\\SPXC3\\textures\\header.png"
local HEADER_PATH="Themes/SPXC3/textures/header.png"
...
local function apply_font_scale(scale)
    ...
    text.set_size(font.title,math.max(1,15*scale))
...
local SETTINGS_FILE="stand_import_v3_spxc3.ini"
menu.clear_settings()
menu.add_setting_color("Stand Accent",0,0,0,170,"Imported primary colour")
menu.add_setting_color("Stand Background",0,0,0,180,"Imported background colour")
...
menu.add_setting_slider("Stand Position X",1252,0,1920,1,"Main-list X coordinate in Stand HUD pixels")
menu.add_setting_slider("Stand Position Y",119,0,1080,1,"Main-list Y coordinate in Stand HUD pixels")
...
local function load_settings()
    ...
    for line in body:gmatch("[^\r\n]+") do
        ...
        elseif key=="position_x" then menu.set_setting("Stand Position X",tonumber(value) or 1252)
        elseif key=="position_y" then menu.set_setting("Stand Position Y",tonumber(value) or 119)
        end
    end
end

local function serialize_settings()
    local accent=setting("Stand Accent") or {r=0,g=0,b=0,a=170}
    local background=setting("Stand Background") or {r=0,g=0,b=0,a=180}
    ...
    local position_x=setting("Stand Position X") or {f_val=1252}
    local position_y=setting("Stand Position Y") or {f_val=119}
    return table.concat({
...
local function reset_settings()
    menu.set_setting("Stand Accent",0,0,0,170)
    menu.set_setting("Stand Background",0,0,0,180)
    menu.set_setting("Stand Width",399)
    menu.set_setting("Stand Rows",20)
    menu.set_setting("Stand Row Height",24)
    menu.set_setting("Stand Position X",1252)
    menu.set_setting("Stand Position Y",119)
...
function draw_menu()
    ...
    local position_x=clamp(position_x_setting and position_x_setting.f_val or 1252,0,1920)*hud_sx
    local position_y=clamp(position_y_setting and position_y_setting.f_val or 119,0,1080)*hud_sy
    ...
    if 150>0 and 2>0 then
        draw.rect_outline(x,content_y,x+width,content_y+body_h,148,0,255,150,5,2)
    end
    ...
    if page_h>0 then
        ...
        if TABS_ENABLED then
            ...
            text.draw(font.small,x+width-10-text.width(font.small,counter),
                page_y+(page_h-text.height(font.small))*0.5,148,0,255,200,counter)
        end
    end
    ...
    for row=0,visible-1 do
        ...
        if item then
            ...
            if 150>0 then
                draw.rect(x,yy+row_h-1,x+width,yy+row_h,148,0,255,math.min(150,80))
            end

            if item.is_header then
                text.draw(font.small,x+10,yy+(row_h-text.height(font.small))*0.5,
                    active and 148 or accent.r,active and 0 or accent.g,active and 255 or accent.b,255,
                ...
                text.draw_ellipsis(font.item,text_x,yy+(row_h-text.height(font.item))*0.5,
                    active and 148 or 148,active and 0 or 0,active and 255 or 255,active and 200 or 200,
                ...
                if toggle_type then
                    ...
                    if toggle_handle and toggle_handle>0 then
                        ...
                        draw.rect_outline(indicator_x-5,yy+row_h*0.5-5,indicator_x+5,yy+row_h*0.5+5,
                            active and 148 or 148,active and 0 or 0,active and 255 or 255,210,1,1)
                        if item.on then draw.rect(indicator_x-3,yy+row_h*0.5-3,indicator_x+3,yy+row_h*0.5+3,
                            active and 148 or accent.r,active and 0 or accent.g,active and 255 or accent.b,255,1) end
                    end
                ...
                elseif action_mark then
                    ...
                    if action_handle and action_handle>0 then
                        ...
                        text.draw(font.small,x+width-10-text.width(font.small,mark),yy+(row_h-text.height(font.small))*0.5,
                            active and 148 or 148,active and 0 or 0,active and 255 or 255,255,mark)
                    end
                end
                if value~="" then
                    ...
                    text.draw(font.value,right-text.width(font.value,value),yy+(row_h-text.height(font.value))*0.5,
                        active and 148 or 148,active and 0 or 0,active and 255 or 255,255,value)
                end
            end
    ...
    if footer_handle and footer_handle>0 then
        ...
        text.draw(font.desc,x+9,desc_y+7,148,0,255,200,description,width-18)
        draw.pop_clip()
    end
