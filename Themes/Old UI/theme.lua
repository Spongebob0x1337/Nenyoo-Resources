...
local SETTINGS_FILE="stand_import_v3_old_ui.ini"
menu.clear_settings()
...
menu.add_setting_slider("Stand Position X",1250,0,1920,1,"Main-list X coordinate in Stand HUD pixels")
menu.add_setting_slider("Stand Position Y",56,0,1080,1,"Main-list Y coordinate in Stand HUD pixels")
...
local function load_settings()
    ...
    for line in body:gmatch("[^\r\n]+") do
        ...
        elseif key=="position_x" then menu.set_setting("Stand Position X",tonumber(value) or 1250)
        elseif key=="position_y" then menu.set_setting("Stand Position Y",tonumber(value) or 56)
        end
    end
end
...
local function serialize_settings()
    ...
    local position_x=setting("Stand Position X") or {f_val=1250}
    local position_y=setting("Stand Position Y") or {f_val=56}
    return table.concat({
...
local function reset_settings()
    ...
    menu.set_setting("Stand Position X",1250)
    menu.set_setting("Stand Position Y",56)
...
function draw_menu()
    ...
    local position_x=clamp(position_x_setting and position_x_setting.f_val or 1250,0,1920)*hud_sx
    local position_y=clamp(position_y_setting and position_y_setting.f_val or 56,0,1080)*hud_sy
