...
local BADGE_FILE="Themes\\Zanium\\textures\\badge.png"
local BADGE_PATH="Themes/Zanium/textures/badge.png"
local BADGE_URL_PATH=""
local BADGE_IS_LOGO=true
local badge_handle=0
local BACKGROUND_FILE="Themes\\Zanium\\textures\\background.png"
local BACKGROUND_PATH="Themes/Zanium/textures/background.png"
...
local SETTINGS_FILE="stand_import_v3_zanium.ini"
menu.clear_settings()
...
menu.add_setting_slider("Stand Width",300,120,1200,1,"Width in Stand's 1920x1080 HUD pixels")
...
menu.add_setting_slider("Stand Position X",1308,0,1920,1,"Main-list X coordinate in Stand HUD pixels")
menu.add_setting_slider("Stand Position Y",255,0,1080,1,"Main-list Y coordinate in Stand HUD pixels")
...
local function load_settings()
    ...
    for line in body:gmatch("[^\r\n]+") do
        ...
        elseif key=="width" then menu.set_setting("Stand Width",tonumber(value) or 300)
        ...
        elseif key=="position_x" then menu.set_setting("Stand Position X",tonumber(value) or 1308)
        elseif key=="position_y" then menu.set_setting("Stand Position Y",tonumber(value) or 255)
        end
    end
end
...
local function serialize_settings()
    ...
    local width=setting("Stand Width") or {f_val=300}
    local rows=setting("Stand Rows") or {f_val=12}
    local row_height=setting("Stand Row Height") or {f_val=33}
    local position_x=setting("Stand Position X") or {f_val=1308}
    local position_y=setting("Stand Position Y") or {f_val=255}
    return table.concat({
...
local function reset_settings()
    ...
    menu.set_setting("Stand Width",300)
    menu.set_setting("Stand Rows",12)
    menu.set_setting("Stand Row Height",33)
    menu.set_setting("Stand Position X",1308)
    menu.set_setting("Stand Position Y",255)
...
function draw_menu()
    ...
    local source_width=clamp(width_setting and width_setting.f_val or 300,120,1200)
    ...
    local position_x=clamp(position_x_setting and position_x_setting.f_val or 1308,0,1920)*hud_sx
    local position_y=clamp(position_y_setting and position_y_setting.f_val or 255,0,1080)*hud_sy
    ...
    if page_h>0 then
        ...
        if TABS_ENABLED then
            ...
            text.draw(font.small,x+width-10-text.width(font.small,counter),
                page_y+(page_h-text.height(font.small))*0.5,255,173,177,255,counter)
        end
    end
        ...
        if item then
            ...
            if item.is_header then
                text.draw(font.small,x+10,yy+(row_h-text.height(font.small))*0.5,
                    active and 255 or accent.r,active and 0 or accent.g,active and 0 or accent.b,255,
                ...
                text.draw_ellipsis(font.item,text_x,yy+(row_h-text.height(font.item))*0.5,
                    active and 255 or 255,active and 0 or 173,active and 0 or 177,active and 255 or 255,
                ...
                if toggle_type then
                    ...
                    if toggle_handle and toggle_handle>0 then
                        ...
                        draw.rect_outline(indicator_x-5,yy+row_h*0.5-5,indicator_x+5,yy+row_h*0.5+5,
                            active and 255 or 255,active and 0 or 173,active and 0 or 177,210,1,1)
                        if item.on then draw.rect(indicator_x-3,yy+row_h*0.5-3,indicator_x+3,yy+row_h*0.5+3,
                            active and 255 or accent.r,active and 0 or accent.g,active and 0 or accent.b,255,1) end
                    end
                ...
                elseif action_mark then
                    ...
                    if action_handle and action_handle>0 then
                        ...
                        text.draw(font.small,x+width-10-text.width(font.small,mark),yy+(row_h-text.height(font.small))*0.5,
                            active and 255 or 255,active and 0 or 173,active and 0 or 177,255,mark)
                    end
                end
                if value~="" then
                    ...
                    text.draw(font.value,right-text.width(font.value,value),yy+(row_h-text.height(font.value))*0.5,
                        active and 255 or 255,active and 0 or 173,active and 0 or 177,255,value)
                end
            end
    ...
    if footer_handle and footer_handle>0 then
        ...
        text.draw(font.desc,x+9,desc_y+7,255,173,177,255,description,width-18)
        draw.pop_clip()
    end
