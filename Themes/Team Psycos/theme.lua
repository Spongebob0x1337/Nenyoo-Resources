-- Team Psycos
-- Compact cyan-to-magenta GTA V menu inspired by the classic Team Psycos UI.

text.set_font_family("Arial")

local TITLE_FONT_FILE="Themes\\Team Psycos\\fonts\\RussoOne-Regular.ttf"
local TITLE_FONT_REL="Themes/Team Psycos/fonts/RussoOne-Regular.ttf"
...
do
    ...
    elseif net and net.get then
        net.get("raw.githubusercontent.com","/google/fonts/main/ofl/russoone/RussoOne-Regular.ttf",function(body)
            if valid_font_data(body) then title_font_downloaded=file.write(TITLE_FONT_FILE,body) end
        end,function() end)
    end
end

text.set_size(font.title,31)
text.set_weight(font.title,900)
text.set_size(font.item,16)
...
local SETTINGS_FILE="team_psycos_theme.ini"
menu.clear_settings()
menu.add_setting_color("Psycos Cyan",53,211,232,255,"Left side of the banner gradient")
menu.add_setting_color("Psycos Magenta",211,75,218,255,"Right side of the banner gradient")
menu.add_setting_slider("Psycos Width",350,280,520,10,"Menu width")
menu.add_setting_slider("Psycos Rows",11,7,18,1,"Visible option rows")
menu.add_setting_slider("Psycos Opacity",76,45,95,1,"List background opacity")
menu.add_setting_slider("Psycos Position",9,0,70,1,"Horizontal position percentage")
menu.add_setting_action("Reset Theme","Reset Team Psycos settings")
...
local function load_settings()
    ...
    for line in body:gmatch("[^\r\n]+") do
        ...
        if key=="highlight" or key=="toggle_on" then
            ...
            if r then
                menu.set_setting(key=="highlight" and "Psycos Cyan" or "Psycos Magenta",
                    tonumber(r),tonumber(g),tonumber(b),tonumber(a))
            end
        elseif key=="width" then menu.set_setting("Psycos Width",tonumber(value) or 350)
        elseif key=="rows" then menu.set_setting("Psycos Rows",tonumber(value) or 11)
        elseif key=="opacity" then menu.set_setting("Psycos Opacity",tonumber(value) or 76)
        elseif key=="position" then menu.set_setting("Psycos Position",tonumber(value) or 9)
        end
    end
end

local function serialize_settings()
    local highlight=setting("Psycos Cyan") or {r=53,g=211,b=232,a=255}
    local toggle_on=setting("Psycos Magenta") or {r=211,g=75,b=218,a=255}
    local width=setting("Psycos Width") or {f_val=350}
    local rows=setting("Psycos Rows") or {f_val=11}
    local opacity=setting("Psycos Opacity") or {f_val=76}
    local position=setting("Psycos Position") or {f_val=9}
    return table.concat({
...
local function reset_settings()
    menu.set_setting("Psycos Cyan",53,211,232,255)
    menu.set_setting("Psycos Magenta",211,75,218,255)
    menu.set_setting("Psycos Width",350)
    menu.set_setting("Psycos Rows",11)
    menu.set_setting("Psycos Opacity",76)
    menu.set_setting("Psycos Position",9)
...
local function activate(item)
    if item.name=="Reset Theme" and item.type==item_type.action then
        reset_settings()
        notify.push("Theme","Team Psycos reset",1)
        return
    end
...
function draw_menu()
    ...
    local cyan=setting("Psycos Cyan") or {r=53,g=211,b=232,a=255}
    local magenta=setting("Psycos Magenta") or {r=211,g=75,b=218,a=255}
    theme.set_body_bg(7,8,10,255)
    theme.set_menu_bg(7,8,10,255)
    theme.set_accent_palette(cyan.r,cyan.g,cyan.b,255)
    if not menu.is_visible() then menu.set_text_editing(false);return end

    local width_setting=setting("Psycos Width")
    local row_setting=setting("Psycos Rows")
    local opacity_setting=setting("Psycos Opacity")
    local position_setting=setting("Psycos Position")
    local width=clamp(width_setting and width_setting.f_val or 350,280,math.max(280,ctx.screen_w()-16))
    local requested_rows=math.floor(row_setting and row_setting.f_val or 11)
    local alpha=math.floor(255*clamp((opacity_setting and opacity_setting.f_val or 76)/100,0.45,0.95))
    local position=clamp(position_setting and position_setting.f_val or 9,0,70)/100
    local count=menu.item_count()
    local sel=menu.selected_index()
    local banner_h=70
    local page_h=27
    local row_h=28
    local rows=math.min(requested_rows,math.max(5,math.floor((ctx.screen_h()-banner_h-page_h-24)/row_h)))
    local start=math.floor(sel/rows)*rows
    local visible=math.min(rows,math.max(0,count-start))
    local height=banner_h+page_h+visible*row_h
    local x=math.floor(clamp(ctx.screen_w()*position,8,math.max(8,ctx.screen_w()-width-8)))
    local y=math.floor(clamp(ctx.screen_h()*0.08,8,math.max(8,ctx.screen_h()-height-8)))
    local ox,oy=menu.drag_header(x,y,width,banner_h)
    x=x+ox;y=y+oy
    menu.set_content_rect(x,y+banner_h+page_h,width,visible*row_h)

    draw.rect(x,y,x+width,y+height,6,7,9,alpha)
    draw.rect_gradient(x,y,x+width,y+banner_h,
        cyan.r,cyan.g,cyan.b,248,magenta.r,magenta.g,magenta.b,248,
        magenta.r,magenta.g,magenta.b,248,cyan.r,cyan.g,cyan.b,248)
    draw.rect(x,y+banner_h-4,x+width,y+banner_h,255,255,255,25)
    draw.rect_outline(x,y,x+width,y+height,16,18,21,225,0,1)
    local title="team psycos"
    local tx=x+(width-text.width(font.title,title))*0.5
    text.draw_outlined(font.title,tx,y+(banner_h-text.height(font.title))*0.5-1,
        255,255,255,255,22,22,25,210,1.3,title)

    local page=menu.page_name() or "Home"
    local page_name=page=="Home" and "Main Menu" or page
    local page_y=y+banner_h
    draw.rect(x,page_y,x+width,page_y+page_h,7,7,9,235)
    text.draw(font.small,x+10,page_y+(page_h-text.height(font.small))*0.5,
        235,235,238,255,page_name)
    ...
    text.draw(font.small,x+width-10-text.width(font.small,counter),
        page_y+(page_h-text.height(font.small))*0.5,235,235,238,255,counter)
    ...
    for row=0,visible-1 do
        ...
        if item then
            ...
            local shade=(row%2)==0 and 11 or 5
            draw.rect(x,yy,x+width,yy+row_h,shade,shade+1,shade+3,alpha)
            if active then draw.rect(x,yy,x+width,yy+row_h,220,220,222,238) end
            if hovered and not active then draw.rect(x,yy,x+width,yy+row_h,110,110,116,90) end
            draw.rect(x,yy+row_h-1,x+width,yy+row_h,215,219,225,55)

            if item.is_header then
                text.draw(font.small,x+10,yy+(row_h-text.height(font.small))*0.5,
                    active and 55 or cyan.r,active and 55 or cyan.g,active and 58 or cyan.b,255,
                    (item.name or ""):gsub("^[%-%s]+",""):gsub("[%-%s]+$",""))
            else
                text.draw_ellipsis(font.item,x+10,yy+(row_h-text.height(font.item))*0.5,
                    active and 52 or 240,active and 52 or 240,active and 56 or 242,255,
                ...
                if toggle_type then
                    draw.circle(indicator_x,yy+row_h*0.5,5,
                        item.on and 43 or 224,item.on and 226 or 40,item.on and 70 or 47,255)
                    draw.circle_outline(indicator_x,yy+row_h*0.5,6,255,255,255,70,1)
                elseif item.type==item_type.selected_tick then
                    draw.circle(indicator_x,yy+row_h*0.5,5,43,226,70,255)
                elseif action_mark then
                    local mark=">"
                    text.draw(font.small,x+width-10-text.width(font.small,mark),yy+(row_h-text.height(font.small))*0.5,
                        active and 55 or cyan.r,active and 55 or cyan.g,active and 58 or cyan.b,255,mark)
                end
                if value~="" then
                    ...
                    text.draw(font.value,right-text.width(font.value,value),yy+(row_h-text.height(font.value))*0.5,
                        active and 55 or 235,active and 55 or 235,active and 58 or 238,255,value)
                end
            end
