-- Terminal theme — bundled player-info panel.
-- Overrides the global "player_panel" overlay by name while the Terminal theme is active.
-- CRT / phosphor-green look: near-black panel, green outline, "//" section headers,
-- segmented health/armour bars, live ped composited into the top-left slot.

local GREEN  = { 88, 255, 136 }   -- phosphor accent
local GREEND = { 40, 120, 70 }    -- dim green
local AMBER  = { 255, 196, 90 }
local RED    = { 255, 90, 90 }
local BLUE   = { 120, 180, 255 }
local DIM    = { 90, 120, 100 }

local function clamp(v, lo, hi) return math.max(lo, math.min(hi, v)) end

-- value-color by content (Yes=green, No=red, Hidden/N/A/-=dim, else blue)
...
overlay.on_draw("player_panel", function()
    ...
    local colL = {
        { "row", "Name",        it.name or "Player", GREEN },
    ...
    -- Near-black green-tinted panel; leave the ped slot UNFILLED (the renderer paints the
    -- backdrop behind the ped). Paint the background around the slot instead.
    local br, bg_, bb, ba = 6, 10, 7, 245
    draw.rect(X0, Y0 + header_h, px1, py0, br, bg_, bb, ba)         -- above slot
    draw.rect(X0, py0, px0, py1, br, bg_, bb, ba)                   -- left strip
    draw.rect(X0, py1, px1, Y0 + H, br, bg_, bb, ba)               -- below slot
    draw.rect(px1, Y0 + header_h, X0 + W, Y0 + H, br, bg_, bb, ba) -- right column

    -- Header bar: "> NAME" with green underline + corner ticks.
    draw.rect(X0, Y0, X0 + W, Y0 + header_h, 4, 7, 5, 255)
    draw.rect(X0 + pad, Y0 + header_h - 1, X0 + W - pad, Y0 + header_h, GREEND[1], GREEND[2], GREEND[3], 160)
    text.draw(font.item, X0 + pad, Y0 + 8, GREEN[1], GREEN[2], GREEN[3], 255, "> " .. string.upper(it.name or "PLAYER"))

    -- Panel outline + corner ticks (terminal frame look).
    draw.rect_outline(X0, Y0, X0 + W, Y0 + H, GREEN[1], GREEN[2], GREEN[3], 150)
    local tk = 8
    draw.rect(X0, Y0, X0 + tk, Y0 + 2, GREEN[1], GREEN[2], GREEN[3], 255)
    draw.rect(X0 + W - tk, Y0 + H - 2, X0 + W, Y0 + H, GREEN[1], GREEN[2], GREEN[3], 255)

    -- Ped slot outline (dim green) + live ped.
    draw.rect_outline(px0, py0, px1, py1, GREEND[1], GREEND[2], GREEND[3], 160)
    ...
    if not ok then
        ...
        text.draw(rfont, px0 + (LW - text.width(rfont, s)) / 2,
            py0 + ph_slot / 2 - 6, GREEND[1], GREEND[2], GREEND[3], 200, s)
    end

    -- Segmented mini bar (Terminal style).
    local function seg_bar(mbx, mby, mbw, frac, col)
        frac = clamp(frac, 0, 1)
        local seg = 10
        local s = (mbw - (seg - 1) * 2) / seg
        local lit = math.floor(frac * seg + 0.5)
        for i = 0, seg - 1 do
            local sx = mbx + i * (s + 2)
            local on = i < lit
            draw.rect(sx, mby, sx + s, mby + 5, col[1], col[2], col[3], on and 255 or 70)
        end
    end
    ...
    local function render(cx, cw, yy, entries)
        for _, e in ipairs(entries) do
            if e[1] == "sec" then
                text.draw(rfont, cx, yy, GREEN[1], GREEN[2], GREEN[3], 200, "// " .. string.upper(e[2]))
                draw.rect(cx, yy + rh - 4, cx + cw - vpad, yy + rh - 3, GREEND[1], GREEND[2], GREEND[3], 120)
            elseif e[1] == "bar" then
                text.draw(rfont, cx, yy, GREEND[1], GREEND[2], GREEND[3], 230, e[2])
                ...
                seg_bar(cbx, yy + 4, cbw, e[3] / e[4], e[5])
            else
                text.draw(rfont, cx, yy, GREEND[1], GREEND[2], GREEND[3], 230, e[2])
